...
- **Static GitHub CIDR fallback (Option B — belt-and-braces).** `nftables/egress.nft` `output_v4` carries three static CIDR rules BEFORE the dynamic `@gh_v4` accept rule: `ip daddr 140.82.112.0/20 accept` (GitHub web/api per `https://api.github.com/meta`) + `ip daddr 192.30.252.0/22 accept` (GitHub legacy) + `ip daddr 143.55.64.0/20 tcp dport 22 accept` (GitHub git-rotating-IP secondary block per `api.github.com/meta` § `git`, port-scoped to TCP/22 — the residual symptom was isolated to `github.com:22` SSH specifically, so web/api to that range still falls through to default-drop). Three-way ordering inside `output_v4` is therefore `static-CIDR → @gh_v4 accept → KEEL_EGRESS_V4_MARKER_START` so the static layer short-circuits during (a) the boot-time-to-first-DNS-reply window, (b) any catastrophic dnsmasq failure mode, and (c) the cache-hit-skips-nftset-add window — dnsmasq emits `nftset add` only on FORWARDED query→reply paths, so a cached reply after a `flush table` reload leaves `@gh_v4` empty until the next forwarded query.

The named sets `gh_v4` / `gh_v6` are declared at table scope with `flags timeout; timeout 600s` (SC-5; hard-coded literal in `egress.nft`, not retunable at this story scope); `flush table` at reload wipes them and dnsmasq re-fills on the next DNS query (typical < 1s) — the in-flight window is covered by Option B. Per SC-4, set names are STATIC per family across all rotating GitHub-class domains; future rotating services with their own published CIDR ranges land as sibling sets in future stories rather than reusing `gh_v<family>`.
