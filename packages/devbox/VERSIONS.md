...
| Tool       | Source                     | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dnsmasq`  | apt (Ubuntu 24.04 default) | In-container DNS authority — listens on 127.0.0.1:53, fail-closed `address=/#/` default; SC-12 allow-mechanism. **Story 2.18: requires `--enable-nftset` build flag** so dnsmasq populates `inet keel_egress` named sets from `nftset=` directives (verify per-bake via `dnsmasq --version \| grep -i nftset`; Debian/Ubuntu's official `dnsmasq` package ships with it as of `2.86-1`). If the bake produces a binary without `--enable-nftset` the Option A dynamic-accept layer fails closed and Option B static GitHub CIDR fallback (`140.82.112.0/20` + `192.30.252.0/22`) becomes the sole egress path for `github.com` / `api.github.com`. |
| `nftables` | apt (Ubuntu 24.04 default) | Layer-3 egress filter — `inet keel_egress` table with `output_v4` + `output_v6` chains both `policy drop`; SC-7 parity. **Story 2.18: declares named sets `gh_v4` / `gh_v6` (`flags timeout; timeout 600s`) at table scope.** Set declarations have been supported since nftables 0.8 (2017); Ubuntu 24.04 ships ≥ 1.0.x.                                                                                                                                                                                                                                                                                                                          |
