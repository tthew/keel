...
const raw: Invariant[] = [
  ...
  },
  {
    ...
    contentHash: '764bb6f87db58d2eb23d385a40273ffb2a4b9e1b6e0b6c561ea201d2d66806dc',
    anchors: ['INV-devbox-egress-contract'],
  },
  {
    id: 'INV-devbox-homedev-named-volume',
    description:
      'Container hardening contract — non-root `dev` user (UID/GID 1000, image USER directive before ENTRYPOINT + runtime `user: 0:0` override with gosu drop to dev at end of entrypoint.sh) + capability bounding set (cap_drop: ALL; cap_add: NET_ADMIN, NET_RAW, NET_BIND_SERVICE, SETUID, SETGID per Story 2.5 SC-4 — nftables netlink + dnsmasq :53 bind + raw-socket probes + gosu setuid(2)/setgid(2) for root→dev drop in entrypoint.sh; SETUID + SETGID added iter-238 when ambient-cap propagation across the image-USER boundary under no-new-privileges=1 was empirically falsified on Docker 29.2 + linux/arm64) + security_opt: no-new-privileges:true + tmpfs posture (/tmp + /var/tmp with noexec,nosuid; sizes parameterised via NFR8a at KEEL_DEVBOX_TMPFS_{TMP,VARTMP}_MB per Story 2.2 .envrc.example) + /home/dev named volume (keel_home_dev, non-toggle under any KEEL_DEVBOX_* setting — never a host bind-mount). Runtime compose-shape check deferred to Story 2.17 / dedicated packages/keel-invariants/src/check-devbox-compose-shape.ts; Story 2.5 registers the substrate-invariant surface. Companion to INV-devbox-dind-available (fork-time Docker runtime) + INV-devbox-egress-contract (fail-closed egress) — the Epic-2 substrate-security trio.',
    sourcePath: 'docs/invariants/devbox-hardening.md',
    contentHash: '6510d297fe93d3d97ab47b5fad8016cc9336867aea0816c0a8516d263ab5111c',
