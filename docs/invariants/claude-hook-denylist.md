...
- **Rule-authoring patterns that interact with substrate-protected token literals — single data points; documented for awareness, not pinned as formal rules.** (a) **Case-glob prefix-then-any matching in `.claude/hooks/*.sh` rule authoring** (Story 2.17 iter-320 observation). Bash `case` patterns like `rm*) ...` match any token starting with `rm` — including `rmdir`, `rm-custom`, etc. New mutation-verb-matching rules in this file SHOULD use word-boundary regex (e.g. `[[ "$normalized" =~ ^rm([[:space:]]|$) ]]`) when verb-class detection requires precision; the L1 install-boundary mutation-verb list (§ L1 install-boundary rule) already follows this discipline. Legacy direct-path case-globs at § Substrate contract retain prefix-then-any semantics deliberately (broader catch by design — `cat *.envrc*` is intended to absorb prefix variants per § Substrate contract). (b) **Rule-authoring self-exclusion globs in `eslint.config.keel-invariants.js`** (Story 2.17 iter-329 observation). ESLint rules and S4 prompt-injection rules whose detection logic matches a literal token (`--no-verify`, `--dangerously-skip-permissions`) need a file-scoped rule override (`files: ['**/eslint-rules/**', '**/keel-invariants/test/**', '**/prompt-injection-rules/**']` → rule `off`) so the rule definition source and its test fixtures don't self-flag. File-scoped rule override is preferred over blanket `ignores:` to preserve `tseslint` + `no-restricted-imports` coverage on the rule source. Both items are awareness-level guidance; promotion to a formal rule requires a second data point.
