...
- **Static GitHub CIDR fallback (Option B — belt-and-braces).** `egress.nft` `output_v4` carries three static CIDR rules BEFORE the dynamic `@gh_v4` accept: `ip daddr 140.82.112.0/20 accept` (GitHub web/api per `https://api.github.com/meta`), `ip daddr 192.30.252.0/22 accept` (GitHub legacy), and `ip daddr 143.55.64.0/20 tcp dport 22 accept` (GitHub git-rotating-IP secondary block per `https://api.github.com/meta` § `git`, port-scoped to TCP/22 because the residual symptom was isolated to `github.com:22` SSH specifically — web/api egress to that range still falls through to default-drop). Three-way ordering inside `output_v4` is therefore `static-CIDR → @gh_v4 accept → KEEL_EGRESS_V4_MARKER_START` so the static layer short-circuits during (a) the boot-time-to-first-DNS-reply window, (b) catastrophic dnsmasq failure modes, and (c) the cache-hit-skips-nftset-add window — dnsmasq only emits `nftset add` on FORWARDED query→reply paths, so a cached reply after `flush table` (atomic reload) leaves `@gh_v4` empty until the next forwarded query. `185.199.108.0/22` (Pages/raw) is intentionally NOT here — Story 2.3's static-pin path already covers it via `raw.githubusercontent.com`.

**Set lifecycle (SC-7).** `flush table inet keel_egress` at the top of `egress.nft` wipes the named sets on every reload; dnsmasq re-fills on the next DNS query (typical < 1s). The brief in-flight window during reload is covered by the Option B static CIDR fallback rules in `output_v4` (no IPv6 static fallback — GitHub's IPv6 ranges are CDN-fronted, and the static-pin path already resolves AAAA records into per-IP accept rules during reload). Per-set timeout default is 600s (SC-5), hard-coded as the literal `timeout 600s` in `egress.nft` at this story scope.
...
The reload primitive composes the dnsmasq config + nftables ruleset by feeding two operator-influenced inputs into `awk -v` assignments: `KEEL_DEVBOX_DNS_UPSTREAM` (env var, expanded into the per-domain `server=/<domain>/<resolver>` render at `reload-egress.sh:307`) and the per-line whitelist contents (read via `mapfile -t domains < <(awk 'NF { print }' "${whitelist_path}")` then stamped into both the dnsmasq directive block and the nftables rule list). PR #230 Round-3 adversarial review (R3-Devbox-D01) walked end-to-end injection demonstrations against this surface and identified two bypass classes:

1. **Env-vector injection.** A value like `KEEL_DEVBOX_DNS_UPSTREAM=$'1.1.1.1\nip daddr 0.0.0.0/0 accept'` (ANSI-C newline) breaks the awk assignment shape and lets the trailing accept-all rule land in the `output_v4` chain — converting fail-closed default-deny into fail-open allow-all for IPv4 egress.
2. **Supply-chain whitelist injection.** A malicious upstream PR adding `foo.com<NL>address=/x.example/2.2.2.2` (or a single-line `foo.com address=evil/2.2.2.2`) to a category fragment under `packages/devbox/whitelist/` would post-`awk 'NF'` survive as one-or-two domain entries and inject `address=/x.example/2.2.2.2` (or similar) directives into the rendered `dnsmasq.conf` — silently re-routing whitelisted-but-unrelated hostnames through an attacker-controlled IP via dnsmasq's host-override path.

Both classes are closed at the substrate by **shape gates at the chokepoint** in `packages/devbox/scripts/reload-egress.sh`:

- `KEEL_DEVBOX_DNS_UPSTREAM` is gated immediately after assignment: `case "$UPSTREAM_RESOLVER" in *[!0-9a-fA-F:.]*|"") ... exit 6 ;; esac` — admits only the IPv4 / IPv6 literal class (digits, hex, `.`, `:`); rejects whitespace, newline, `=`, `/`, `#`, brackets, ports. Defense-in-depth mirror at operator-edit time lives in `packages/devbox/scripts/env-check.sh § SHAPE_IP_LITERAL` (same case pattern; emits a shape-violation report and exits 2 before `pnpm devbox:start` reaches the reload step).
- Per-domain whitelist entries are gated post-`mapfile`: `case "$domain" in ""|*[!A-Za-z0-9.-]*) ... exit 6 ;; esac` — admits only the LDH class `[A-Za-z0-9.-]`; rejects empty, whitespace, newline, `=`, `/`, and any other shell- or directive-meta character. Matches the `whitelist.default.txt § Syntax` "no wildcards at 1.0 (explicit domains only)" surface; forks needing `*`-prefixed entries pursue the AMEND path (extend the class to `[A-Za-z0-9.*-]+` here AND in `reload-egress.sh § Per-domain shape gate` in lockstep).

**Residual classes (accepted).** The substrate hook surface (`.claude/hooks/block-secret-access.sh`) is a regex-based gate against deny-list verbs reading secret-class paths; it does not run a sandboxed shell evaluator. Round-3 R3-H49 (`bash -c "$(echo 'Y2F0...'|base64 -d)"` static-vs-dynamic limit) and R3-H48 (verb-trailing no-space class) are anchored as WONTFIX with inline comment blocks — runtime evaluation is out of scope for a regex gate, and loosening the verb-trailing class to admit `<`/`>` would false-positive on legitimate `cat<<HEREDOC` forms. `sshd_config` defense-in-depth (R3-D03) and the `NET_RAW` cap_add justification (R3-D04) are documented in `docs/invariants/devbox-hardening.md` § Defense-in-depth gaps and operator-workstation verification respectively — the egress firewall (this invariant) is the load-bearing layer.
