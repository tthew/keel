...
- **Clause 2 (always; root-gated)** — `[ "$(id -u)" -ne 0 ] || ( nft list chain inet keel_egress output_v4 2>/dev/null | grep -q 'policy drop' && nft list chain inet keel_egress output_v6 2>/dev/null | grep -q 'policy drop' )` — probes the egress nftables chains. Liveness semantics: clause 2 success = BOTH the `output_v4` AND `output_v6` chains in the `inet keel_egress` table are present in the kernel netfilter state AND each carries `policy drop` as its base disposition. Catches the manual-flush / image-runtime-tamper failure mode where a chain disappears mid-run AND the policy-flip case where a `CAP_NET_ADMIN`-holding caller replaces `policy drop` with `policy accept` (or where a partial `nft -f` reload lands v4 but fails v6). PR #230 review FIX-5 → FIX-10 evolution: FIX-5 verified `output_v4` presence only; FIX-10 (review A7) extends to (a) v6 coverage — egress.nft:84-149 ships v6 rules so v6 chain deletion was previously invisible — and (b) policy-drop verification on both chains, matching the verification recipe pinned at `packages/devbox/nftables/egress.nft:27-28`. Chain-content drift (rule deletion under `CAP_NET_ADMIN` against an otherwise-intact policy-drop chain) remains uncovered by the inline probe per PR #230 review A7 threat-model caveat — an attacker with `CAP_NET_ADMIN` already controls the firewall, so additional inline coverage adds little signal; deferred to a future smoke-script extraction (`healthcheck-egress.sh`). Complements `start-egress.sh`'s boot-time fail-closed posture (entrypoint exits if init fails). The `nft list` netlink call requires `CAP_NET_ADMIN` in the calling thread's effective set; under `no-new-privileges:true` (compose:239) the file-cap on `/usr/sbin/nft` (Dockerfile:293) is masked on exec, so a non-root caller cannot list rules even with file caps. The `[ "$(id -u)" -ne 0 ]` short-circuit gates the probe on root: when `id -u` is non-zero the LHS exits 0, bypassing the chain queries entirely; when `id -u` is 0 the LHS exits 1 and the subshell runs both `nft list … | grep -q 'policy drop'` pipelines. Docker `HEALTHCHECK` probes execute under `container.Config.User`, set to `"0:0"` by `services.devbox.user: '0:0'` (compose:237), so the substrate probe runs as root with `CAP_NET_ADMIN` available from `cap_add` (compose:194). Forks overriding `user:` to a non-root identity preserve the dnsmasq + sshd liveness signal but lose the chain probe — they are responsible for restoring it via a fork-specific compose-override per § Fork extension contract (e.g. a setuid wrapper or a sentinel-file probe written by start-egress.sh).
...
```
dig @127.0.0.1 -p 53 +short +time=3 +tries=1 api.github.com >/dev/null && { [ "$(id -u)" -ne 0 ] || ( nft list chain inet keel_egress output_v4 2>/dev/null | grep -q 'policy drop' && nft list chain inet keel_egress output_v6 2>/dev/null | grep -q 'policy drop' ); } && { [ "${KEEL_DEVBOX_SSH:-false}" != "true" ] || nc -z -w 2 127.0.0.1 2222; }
```

The probe deliberately does NOT exercise whitelist membership or pubkey auth — both would add fragility without adding health signal. Whitelist drift would false-positive the healthcheck; pubkey-auth probing would require committed test keys that weaken the trust model. The chain probe verifies presence + base policy on both `output_v4` and `output_v6` (not rule-set freshness): `nft list chain` succeeds against either a current or stale ruleset, so this clause does NOT detect a failed `pnpm devbox:whitelist` reload (the prior ruleset stays active per `reload-egress.sh:289-293` fail-closed posture); it catches the "chain entirely missing" surface AND the "policy drop replaced with accept" surface that `start-egress.sh`'s boot-time gate cannot reach mid-run. Rule-deletion drift inside an otherwise-intact policy-drop chain remains uncovered by the inline probe per PR #230 review A7 threat-model caveat — deferred to a future `healthcheck-egress.sh` smoke-script extraction.
...
| `nft list chain`                                                                  | `1`                                  | `No such file or directory` (chain missing) or `Operation not permitted` (caller lacks `CAP_NET_ADMIN`); stderr suppressed via `2>/dev/null` redirect. |
| `nft list chain ... \| grep -q 'policy drop'`                                     | `0`                                  | Pipeline last-stage exits 0 → chain present AND its base disposition is `policy drop`. |
| `nft list chain ... \| grep -q 'policy drop'`                                     | `1`                                  | Either `nft list chain` produced no stdout (chain missing / `EPERM`) → `grep` exits 1 on empty stdin, OR chain present but base disposition is not `policy drop` (e.g. `policy accept`) → `grep` exits 1 on no match. Pipeline exit = `grep`'s exit per POSIX `$?` (no `pipefail`). |
| Composed shell (`dig … && { [ id ] \|\| ( nft … \| grep … && nft … \| grep … ); } && { [ ssh ] \|\| nc -z … ; }`) | `0`                                  | All three clauses succeed per POSIX `&&` short-circuit. |
...
- **`KEEL_DEVBOX_SSH=false` (or unset) mode:** `${VAR:-false}` expands to `false`; `[ "false" != "true" ]` exits 0; chain past `||` short-circuits the `nc -z` call. Only dnsmasq + nft (v4 + v6 policy-drop) probes run.
...
- No removing the chain-presence + policy-drop clause under root-running probes (forks running probes as root MUST keep `nft list chain ... | grep -q 'policy drop'` for BOTH `output_v4` and `output_v6`); forks running probes as non-root MUST add an equivalent fork-specific replacement (sentinel-file or setuid-wrapper) per § Probe tooling.
