...
- 2026-04-19: Story 1.1 Tasks 1–8 all shipped on `feat/story-1-1-monorepo-scaffold` → PR #217 (Draft). Next action: transition Draft→Open, then EPIC_DONE halt. Task 1 folded in `typescript@5.7.3` + `turbo@2.3.3` root devDeps. Task 2 authored `tsconfig.base.json` with `composite + noEmit` at base — per-package tsconfigs set `noEmit: false` to emit, avoiding composite/noEmit conflict. Task 3 authored `turbo.json` (turbo 2.x `tasks` schema, NOT legacy `pipeline`) and root `tsconfig.json` solution file with 16 references. Per-package tsconfig pattern: `{ extends: "../../tsconfig.base.json", compilerOptions: { composite: true, noEmit: false, outDir: "./dist", rootDir: "./src" }, include: ["src/**/*"], references: [] }`. Workspace has **16 members** (not 15) — `@keel/web` + 14 business packages + `@keel/create-keel-app`. Root `keel` is the 17th pnpm-visible project but not a workspace member.
