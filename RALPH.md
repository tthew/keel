...
- 2026-04-19: Story 1.1 COMPLETE — PR #217 Open, awaiting user merge. Halted EPIC_DONE. Next mini-epic is Story 1.2 (`1-2-packages-keel-invariants-bootstrap-shared-eslint-prettier-commitlint-configs`) — starts a NEW branch off freshly-merged `main` (`git fetch origin main && git checkout -b feat/story-1-2-... origin/main`; worktree can't own `main`). Story 1.2 will move `tsconfig.base.json` INTO `packages/keel-invariants/` per architecture line 1240 — don't treat current repo-root location as permanent. Carry-forward facts for 1.2+: workspace has **16 members** (`@keel/web` + 14 business packages + `@keel/create-keel-app`); root `keel` is pnpm-visible but NOT a workspace member; `typescript@5.7.3` + `turbo@2.3.3` pinned at root; per-package tsconfig pattern `{ extends: "../../tsconfig.base.json", compilerOptions: { composite: true, noEmit: false, outDir: "./dist", rootDir: "./src" }, include: ["src/**/*"], references: [] }`; `turbo.json` uses 2.x `tasks` schema (NOT legacy `pipeline`).
...
- 2026-04-19: **First story-implementation mini-epic halted** — closed Story 1.1 as EPIC_DONE while PR #217 was Open (CLEAN, MERGEABLE, no reviews, `statusCheckRollup: []` because no CI workflows exist yet). 9 commits on `feat/story-1-1-monorepo-scaffold`: 1 story-spec + 8 task commits. Extends precedent: multi-task story-implementation PRs also halt EPIC_DONE on Open+clean+no-reviews once all tasks + final gate are done. Before `gh pr ready`, rewrote PR title (docs → feat) and body (now describes all 8 tasks + scope variance) so the Open review-state matches the actual diff.
