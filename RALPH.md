...
- 2026-04-19: Story 1.1 implementation in flight on `feat/story-1-1-monorepo-scaffold` → PR #217 (Draft). Tasks 1–6 shipped. Remaining: Tasks 7–8 (`pnpm -w typecheck` + cache verification, structural invariants check). Task 1 folded in `typescript@5.7.3` + `turbo@2.3.3` root devDeps. Task 2 authored `tsconfig.base.json` with `composite + noEmit` at base — per-package tsconfigs in Task 4 set `noEmit: false` to actually emit, avoiding composite/noEmit conflict. Task 3 authored `turbo.json` (turbo 2.x `tasks` schema, NOT the legacy `pipeline` field) and root `tsconfig.json` solution file with 16 references — references will fail `tsc -b` until Task 5 lands `apps/web` tsconfig; Task 7 is the verification gate. Task 4 pattern for per-package tsconfig (reuse in Task 5 apps/web): `{ extends: "../../tsconfig.base.json", compilerOptions: { composite: true, noEmit: false, outDir: "./dist", rootDir: "./src" }, include: ["src/**/*"], references: [] }`. Task 6 ran `pnpm install` clean (43.8s, exit 0); `pnpm-lock.yaml` committed. Workspace has **16 members** (not 15 as the story text said) — `@keel/web` + 14 business packages + `@keel/create-keel-app`. Root `keel` is the 17th pnpm-visible project but not a workspace member.
