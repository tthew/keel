...
- 2026-04-19: Story 1.1 implementation in flight on `feat/story-1-1-monorepo-scaffold` → PR #217 (Draft). Tasks 1–2 shipped. Remaining: Tasks 3–8 (turbo.json + root tsconfig solution, 15 package shells, apps/web shell, `pnpm install`, `pnpm -w typecheck` + cache verification, structural invariants check). Task 1 folded in `typescript@5.7.3` + `turbo@2.3.3` root devDeps. Task 2 authored `tsconfig.base.json` with `composite + noEmit` at base — per-package tsconfigs in Task 4 MUST set `noEmit: false` to actually emit, otherwise `tsc -b` will hit composite/noEmit conflict.
