...
- [ ] **Task 3 — `sync` subcommand** (AC 2, AC 4, SC-4, SC-7, SC-17)
  ...
  - [ ] Subtask 3.4: compute diff — if `${PREVIOUS_COMPOSED}` exists, run `diff --new-line-format='+%L' --old-line-format='-%L' --unchanged-line-format='' "${PREVIOUS_COMPOSED}" "${tempfile}"` into a variable; otherwise every line is a `+` addition (first-sync-after-boot case per SC-18). Count additions + removals.
  ...
  - [ ] Subtask 3.8: emit the diff summary to stdout per SC-7 format. If zero additions and zero removals, emit only the header + `(0 added, 0 removed)` parenthetical.
  - [ ] Subtask 3.9: exit 0.
...
- [ ] **Task 7 — `start-egress.sh` extension for `whitelist.local.txt`** (AC 1, SC-4, SC-14)
...
- [ ] **Task 8 — `whitelist.local.txt` gitignore + AGENTS.md per-fork override doc** (AC 1, SC-2, SC-3)
  - [ ] Subtask 8.1: edit `.gitignore` at repo root — inside the `# Environment / secrets` block (immediately after the `!packages/devbox/.secrets.example` bang-negation at line ~44), append:
...
- [ ] **Task 11 — Change Log v1.0 + sprint-status flip** (lifecycle hygiene)
  - [ ] Subtask 11.1: add v1.0 entry to this story file's Change Log section (iter number, draft summary, Status transition `backlog → ready-for-dev`, sprint-status transition).
  - [ ] Subtask 11.2: flip `_bmad-output/implementation-artifacts/sprint-status.yaml` row `2-4-whitelist-source-of-truth-…` from `backlog` → `ready-for-dev`.
  - [ ] Subtask 11.3: append a new `# last_updated: 2026-04-22 Story-2-4-ready-for-dev UTC` comment line at the top of sprint-status.yaml (match Story 2.3 precedent format).
  - [ ] Subtask 11.4: ensure no scope creep (SC-19) — this story delivers EXACTLY `whitelist.sh` CLI + `start-egress.sh` 5-line extension + `package.json` one-line script wiring + `.gitignore` + `AGENTS.md` + `README.md` doc updates + `whitelist.default.txt` header-comment refresh. Stories 2.5 (hardening), 2.6 (host-side pnpm wrappers), 2.13 (healthcheck) remain in `backlog` until their turn.
