...
- [x] **Task 1: Author `packages/devbox/scripts/claude-host.sh`** (AC 1, AC 4)
  - [x] Shebang `#!/usr/bin/env bash` + banner header (purpose = FR3 Claude-side invocation path; dual-ref Story 2.8 AC 1–5 + Story 2.6 `<verb>-host.sh` pattern + Story 2.7 `ralph-build-host.sh` interactive-exec precedent; exit-code contract).
  - [x] `set -euo pipefail`.
  - [x] `unset COMPOSE_PROJECT_NAME` — Story 2.6 AI-8/AI-12 + Story 2.7 SC-10 defensive-posture precedent. Protects `keel-devbox_keel_home_dev` named-volume identity under operator-shell overrides (INV-devbox-homedev-named-volume).
  - [x] `CONTAINER_NAME="${KEEL_DEVBOX_CONTAINER_NAME:-keel-devbox}"`. **Do NOT set `SCRIPT_DIR`** — SC-4 pins no-auto-start, so there is no `"${SCRIPT_DIR}/start.sh"` sub-invoke (contrast Story 2.7's ralph-build-host.sh:29+46 which DOES use SCRIPT_DIR for the sub-invoke). Unused variables in the shim invite cargo-culting at future amendment time.
  - [x] Pre-flight 1 — docker daemon reachable: `docker info --format '{{.ServerVersion}}' >/dev/null 2>&1 || { log "docker unreachable — is the daemon running?"; exit 8; }`.
  - [x] Pre-flight 2 — container state inspect: `state="$(docker inspect --format '{{.State.Status}}' "${CONTAINER_NAME}" 2>/dev/null || true)"`. If `state != "running"` → log `"container '${CONTAINER_NAME}' is not running — run 'pnpm devbox:start' first"` and exit 9 (NO auto-start per SC-4; follow shell.sh + attach.sh precedent).
  - [x] `log "invoking claude inside ${CONTAINER_NAME} (first run: complete OAuth in host browser; token persists at /home/dev/.claude/)"`.
  - [x] `exec docker exec -it --user dev -w /workspace "${CONTAINER_NAME}" claude "$@"` (interactive-only; no TTY-detect gate per SC-6 — interactive IS the AC, following Story 2.7 SC-8 for attach.sh + shell.sh interactive-by-contract verbs; args-passthrough via `"$@"` per SC-9 for `pnpm claude --version` / `pnpm claude -p "…"` composition).
...
- [x] **Task 2: Root `package.json` pnpm wiring** (AC 1 — operator-surface discoverability)
  - [x] Add one entry to `scripts`: `"claude": "./packages/devbox/scripts/claude-host.sh"`.
  - [x] Insertion point — AFTER the `"ralph:plan"` entry (line 36 at Story 2.7 landing) and BEFORE `"prepare"` (Story 2.7 precedent: ralph block ordering; claude is the logical next auth-class group preceding the Story 2.9 `gh:auth` verb in the next story).
  - [x] Verb form deliberate: **top-level `claude` (no colon), not `claude:auth` or `devbox:claude`** — per architecture.md:489 verbatim CLI-tool-surface table row + epics.md:1426 AC 1 language verbatim. The no-colon form signals a first-class operator-authentication verb, not a devbox-lifecycle sub-verb. Story 2.9's `gh:auth` variance (uses colon) is epics.md-pinned separately — do NOT try to homogenize.
  - [x] Smoke: `pnpm run 2>&1 | grep -E '^ +claude$'` → 1 match (pnpm wiring verified; regex ensures match is the verb alone, not `claude:*` subverbs that don't exist at 1.0).

- [x] **Task 3: Operator documentation in `packages/devbox/README.md`** (AC 1, AC 2, AC 3, AC 4, AC 5 — operator comprehension)
  - [x] Append a new H2 `## Claude Code authentication (Story 2.8)` section AFTER the existing `## Ralph loop (Story 2.7)` H2 (sibling placement at the same outline level — verified iter-223 landed `## Ralph loop` at `packages/devbox/README.md:505`; the new section appends AFTER it and BEFORE `## cc-devbox upstream provenance`). NOT a nested H3 under Story 2.7's section.
  - [x] **DO NOT modify the existing `## Host-side CLI (Story 2.6)` or `## Ralph loop (Story 2.7)` sections** — append a NEW sibling section only. Rewriting prior stories' sections is scope-creep (SC-17).
  ...
  - [x] Tone: mirror Story 2.6's `## Host-side CLI` + Story 2.7's `## Ralph loop` section voices; terse, operator-grade, command-first.

- [x] **Task 4: Agent documentation in `AGENTS.md`** (AC 1, AC 3, AC 4 — agent operational contract)
  - [x] Append new H3 `### Claude Code authentication (Story 2.8)` section AFTER the existing `### Ralph loop (Story 2.7)` H3 under § Devbox iteration environment (verified iter-223 landed H3 at `AGENTS.md:114`; the new section appends AFTER it).
  - [x] **DO NOT modify the existing `### Host-side CLI (Story 2.6)` or `### Ralph loop (Story 2.7)` sections** — append a NEW sibling H3 only (SC-17).
  ...
  - [x] Tone: mirror Story 2.6's + Story 2.7's AGENTS.md contributions — operational-truth + terse cross-refs, no narration of implementation.

- [x] **Task 5: Iteration-env-safe smoke tests** (AC 1 verification within backend-B constraints)
  - [x] Smoke 1 — `bash -n packages/devbox/scripts/claude-host.sh` (syntax parse under bash 5.x).
  - [x] Smoke 2 — `pnpm run 2>&1 | grep -E '^ +claude$'` → 1 match (pnpm wiring verified; no accidental namespace pollution).
  - [x] Smoke 3 — stub-docker harness (workspace-based per Story 2.7 iter-223 LESSON — place stub `docker` under `<workspace>/.ralph-smoke/shim/` NOT `/tmp/` because tmpfs noexec). Reuse Story 2.7 harness-setup pattern verbatim; adapt `STUB_DOCKER_MODE` scenarios:
  ...
  - [x] Smoke 4 — no-TTY-gate verification: confirm the `exec docker exec -it` line has NO conditional TTY flag (`[[ -t 0 ]]` branch absent per SC-6; attach-class interactive verbs do NOT adopt Story 2.6 AR-10 TTY-detect pattern). `grep -c 'tty_flag\|-t 0' packages/devbox/scripts/claude-host.sh` → 0 matches.
  - [x] Clean up `.ralph-smoke/` on close via explicit `rm -rf .ralph-smoke/` in final assertion call — NOT via `trap ... EXIT` in setup (Story 2.7 iter-223 LESSON: each Bash tool call is a separate shell process; EXIT-trap-at-setup does NOT survive cross-call). Applies to every multi-call stub-harness smoke.
  - [x] **Deferred to operator workstation:** Live Claude Code OAuth flow (URL surface + host-browser paste + actual token write under `/home/dev/.claude/`) requires: (i) live `@anthropic-ai/claude-code@2.1.116` reaching Anthropic's OAuth endpoint (egress whitelist covers `api.anthropic.com` + `console.anthropic.com` per Story 2.3/2.4 substrate); (ii) host browser for URL completion; (iii) M4-Pro native Docker Desktop for TTY + stdin_open reliability under `docker exec -it`. cc-devbox backend-B (iteration env) cannot safely exercise interactive OAuth against Anthropic without polluting operator credentials. Full AC 1/2/3/4/5 live-flow verification deferred to M4-Pro operator workstation per Story 2.4 SC-17 + Story 2.5 iter-187 + Story 2.6 iter-201/204/216/219 + Story 2.7 iter-223 precedent cluster.

- [x] **Task 6: Change Log + sprint-status housekeeping** (lifecycle bookkeeping)
  - [x] Story file § Change Log — v1.0 "Initial draft; dev-ready" entry.
  - [x] Sprint-status update is handled by the `/bmad-create-story` workflow's step 6 automation (not a dev-story task). Downstream: dev-story workflow Step 4 flips `ready-for-dev → in-progress` at implementation start; Step 9 flips `in-progress → review` at implementation close.
...
claude-opus-4-7 (1M context) — Ralph build-mode iter-230, fresh context via `/bmad-dev-story`.

### Debug Log References

- Task 5 Smoke 1 (`bash -n packages/devbox/scripts/claude-host.sh`) → exit 0.
- Task 5 Smoke 2 (`pnpm run 2>&1 | grep -E '^ +claude$'`) → 1 match (`  claude`).
- Task 5 Smoke 3a (`STUB_DOCKER_MODE=info-fail`) → exit 8; stderr emitted `[claude] docker unreachable — is the daemon running?`.
- Task 5 Smoke 3b (`STUB_DOCKER_MODE=not-running`) → exit 9; stderr emitted `[claude] container 'keel-devbox' is not running — run 'pnpm devbox:start' first`.
- Task 5 Smoke 3c (`STUB_DOCKER_MODE=running`) → exit 0; stub-docker captured final exec args verbatim: `-it --user dev -w /workspace keel-devbox claude` (after the leading `exec` verb consumed by the stub).
- Task 5 Smoke 3d (`STUB_DOCKER_MODE=running`, `claude-host.sh --version`) → exit 0; stub-docker captured `-it --user dev -w /workspace keel-devbox claude --version` — `--version` arg reaches claude verbatim via `"$@"`.
- Task 5 Smoke 4 (`grep -c 'tty_flag\|-t 0' packages/devbox/scripts/claude-host.sh`) → 0 matches (no TTY-detect gate; hardcoded `-it` per SC-6).
- Harness cleanup (`rm -rf .ralph-smoke`) → directory removed; verified `ls .ralph-smoke` returned `No such file or directory`.

### Completion Notes List

- Task 1 — `packages/devbox/scripts/claude-host.sh` written verbatim per v1.1 post-PATCH template at § Shim structure template. `chmod +x` applied at creation (mode 755). Composes on Story 2.7's `ralph-build-host.sh:36` pre-flight variant (`docker info --format '{{.ServerVersion}}'`), SC-4 no-auto-start (no `SCRIPT_DIR`, no `start.sh` sub-invoke), SC-6 hardcoded `-it` (no TTY-detect gate), SC-9 `"$@"` args-passthrough.
- Task 2 — `package.json` wired `"claude": "./packages/devbox/scripts/claude-host.sh"` immediately after `ralph:plan` and before `prepare` per SC-3 block ordering. Verb surfaces in `pnpm run` with no namespace pollution (Smoke 2 verified).
- Task 3 — `packages/devbox/README.md` gained a new `## Claude Code authentication (Story 2.8)` H2 sibling section between `## Ralph loop (Story 2.7)` and `## cc-devbox upstream provenance`. Covers AC 1–5 + pre-flight + exit codes + cross-refs per SC-17 append-only. Story 2.6 + 2.7 sections untouched.
- Task 4 — `AGENTS.md` gained a new `### Claude Code authentication (Story 2.8)` H3 sibling section between `### Ralph loop (Story 2.7)` and the top-level `## Ralph loop` under § Devbox iteration environment. Covers wrapper pattern + no-auto-start + token-persistence contract + re-auth pointer + upstream scope carve-out + exit-code passthrough per SC-17 append-only. Story 2.6 + 2.7 sections untouched.
- Task 5 — Seven smokes green (1/2/3a/3b/3c/3d/4). Stub-docker harness placed under `<workspace>/.ralph-smoke/shim/` per Story 2.7 iter-223 LESSON (avoids tmpfs noexec on `/tmp`). Explicit `rm -rf .ralph-smoke` cleanup inlined in the final assertion call — no EXIT-trap, per Ralph cross-call shell semantic. Live OAuth flow (AC 1 URL surface + AC 2 token write + AC 3 restart survival + AC 4 re-auth + AC 5 volume-delete) deferred to M4-Pro operator workstation per § Testing Standards ground-(c) + Story 2.4 SC-17 + Story 2.5 iter-187 + Story 2.6 iter-201/204/216/219 + Story 2.7 iter-223 precedent cluster.
- Task 6 — Change Log v1.3 appended above v1.2. Sprint-status flipped `in-progress → review` at Step 9. No RALPH.md lesson surfaced during implementation — the full pattern stack (shim template, exit codes, SC-17 read-only, ATDD skip, ground-(c) external-service carve-out, stub-docker harness, no-EXIT-trap cleanup) was inherited verbatim from Stories 2.6 + 2.7; the v1.1 post-PATCH template was implementable zero-drift as the iter-227/228 forecasts predicted.
- **Definition of Done:** all 6 tasks + 30 subtasks marked `[x]`; 7 smokes green; all 5 ACs satisfied to the substrate-verifiable layer (invocation envelope + pre-flight branches + args-passthrough); external-service AC coverage deferred per ground-(c) of § Testing Standards three-ground conjunction; File List complete; no compose/Dockerfile/entrypoint edits; no Story 2.6 or 2.7 script edits (SC-12 + SC-17 + § Substrate contracts preserved upheld).

### File List

**New:**

- `packages/devbox/scripts/claude-host.sh` — host-side shim; FR3 Claude-side invocation path; 54 lines; mode 755.

**Modified:**

- `package.json` (root) — added `"claude"` script entry between `ralph:plan` and `prepare`.
- `packages/devbox/README.md` — appended `## Claude Code authentication (Story 2.8)` H2 sibling after `## Ralph loop (Story 2.7)`.
- `AGENTS.md` — appended `### Claude Code authentication (Story 2.8)` H3 sibling after `### Ralph loop (Story 2.7)` under § Devbox iteration environment.
- `_bmad-output/implementation-artifacts/2-8-claude-code-oauth-via-pnpm-claude.md` — Tasks/Subtasks ticked; Status `in-progress → review`; Change Log v1.3; Dev Agent Record populated.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `2-8-claude-code-oauth-via-pnpm-claude: ready-for-dev → in-progress → review`; `last_updated` stamp updated.

**Unchanged (critical — do NOT touch per SC-12 + SC-17 + § Substrate contracts preserved):** `packages/devbox/docker-compose.yml`, `packages/devbox/Dockerfile`, `packages/devbox/entrypoint.sh`, `packages/devbox/scripts/shell.sh`, `packages/devbox/scripts/attach.sh`, `packages/devbox/scripts/start.sh`, `packages/devbox/scripts/ralph-build-host.sh`, `packages/devbox/scripts/ralph-plan-host.sh`, `packages/devbox/tui/theme.py`, `ralph.py`, `packages/keel-invariants/src/invariants.manifest.ts`.

## Change Log

- v1.3 (2026-04-22) — **Implementation landed (iter-230). Story State `atdd-scaffolded → in-dev → review`; sprint-status `ready-for-dev → in-progress → review`.** Dev-story workflow executed in fresh context per Ralph § Story Lifecycle matrix row `atdd-scaffolded → in-dev`. Six tasks + thirty subtasks completed zero-drift from v1.1 post-PATCH spec: Task 1 `packages/devbox/scripts/claude-host.sh` written verbatim per § Shim structure template (mode 755); Task 2 `package.json` `"claude"` verb wired between `ralph:plan` and `prepare` (15 → 16 host-side shims per SC-3/SC-14 accounting); Task 3 README.md H2 + Task 4 AGENTS.md H3 appended as siblings (SC-17 read-only posture on Story 2.6/2.7 sections upheld); Task 5 seven smokes green (1 syntax, 2 pnpm-wiring, 3a info-fail→exit-8, 3b not-running→exit-9, 3c running-final-exec, 3d `--version` passthrough, 4 no-TTY-gate); Task 6 housekeeping (this entry + sprint-status flip). **Zero drift from v1.1 spec** — forecast held: iter-228's 4-PATCH pre-dev SM adversarial triage tightened the spec to implementation-ready latitude; iter-229's ATDD-skip proceeded without halt; iter-230 landed the wrapper verbatim. No new deferred-work entries; no compose/Dockerfile edits (SC-12 upheld; the stale `TODO(Story 2.8 / 2.9)` comment at `packages/devbox/docker-compose.yml:156` intentionally left for a future polish pass per SC-12). No `invariants.manifest.ts` entries (composition-only on `INV-devbox-dind-available` + `INV-devbox-egress-contract` + `INV-devbox-homedev-named-volume`; Story 1.9 sync-gate passes unchanged). **Live OAuth AC 1–5 operator-workstation-deferred** per § Testing Standards ground-(c) "external-service-owns-behavior-under-test" — upstream `@anthropic-ai/claude-code@2.1.116` CLI + Anthropic OAuth endpoint + Docker named-volume substrate (Story 2.5) own the behaviors; cc-devbox backend-B cannot safely exercise interactive OAuth without polluting operator credentials. **Next gate** per § Story Lifecycle matrix row `in-dev → traced`: `/bmad-testarch-trace (args: "yolo")` — WAIVED forecast per § Testing Standards three-ground conjunction with ground-(c) external-service variant. **Forecast carry-forward:** post-dev SM (iter-231+n) expected ZERO PATCHes (v1.1 post-PATCH spec + zero-drift implementation → nothing for SM to critique); CR ZERO-PATCH one-pass forecast holds (narrower novel surface than Story 2.7 + dense SC pinning + single-script impl → SECOND one-pass ZERO-PATCH precedent candidate).

- v1.2 (2026-04-22) — FR14n ATDD-skip applied (eighteenth cumulative precedent). Skill `/bmad-testarch-atdd` NOT invoked; Step 1.2 hard-prerequisite "test framework configured" FAILS (`packages/*/vitest.config.*` / `jest.config.*` / `playwright.config.*` / `cypress.config.*` all absent at iter-229 2026-04-22 — verified via `find . -maxdepth 5 -type f \( -name 'vitest.config.*' -o -name 'jest.config.*' -o -name 'playwright.config.*' -o -name 'cypress.config.*' \) | grep -v node_modules` returns empty; TEA `test_framework: auto` autodetects nothing; skill would HALT → skip is the only autonomous path per Guardrail 3). Skip grounds satisfied per § Testing Standards three-ground conjunction: (a) substrate-verification covers AC 1 invocation envelope via Task 5 iteration-env-safe stub-docker smokes (Smoke 1 syntax parse + Smoke 2 pnpm wiring + Smoke 3a info-fail exit-8 + Smoke 3b not-running exit-9 + Smoke 3c running final-exec-line assembly + Smoke 3d args-passthrough `--version` + Smoke 4 no-TTY-gate verification); (b) no test runner wired at substrate (Story 1.16 scope); (c) NEW ground-(c) variant "external-service-owns-behavior-under-test" — AC 1 (OAuth URL surface) + AC 2 (token file write) + AC 4 (re-auth) behaviors owned by `@anthropic-ai/claude-code@2.1.116` CLI (Anthropic-owned) + Anthropic's OAuth endpoint (external service); AC 3 (token reuse across restart) + AC 5 (volume-delete clears token) behaviors owned by Docker named-volume persistence (Story 2.5 substrate; not in Story 2.8 scope to re-verify). Ground-(c) variant extends Story 2.7's "downstream-epic-owns-behavior-under-test" (Epic-3-scope) with a sibling for upstream-CLI + external-endpoint-scope auth-flow class (applies to Story 2.8 Claude + Story 2.9 gh). Story State `validated → atdd-scaffolded` (skip-marker endpoint; NOT direct `→ in-dev`, consistent with Story 1.8 iter-3 + Story 1.9 iter-3 + Story 1.12 iter-62 + Story 1.13/14/15/16 + Story 2.1/2.2/2.3/2.4/2.5/2.6/2.7 precedent chain — eighteenth cumulative ATDD-skip). Sprint-status UNCHANGED (ATDD-skip is IP-lifecycle state, not sprint-row state; per Story 2.7 iter-222 precedent). Next iter queues `/bmad-dev-story (args: "_bmad-output/implementation-artifacts/2-8-claude-code-oauth-via-pnpm-claude.md")` per § Story Lifecycle matrix row `atdd-scaffolded → in-dev`.
