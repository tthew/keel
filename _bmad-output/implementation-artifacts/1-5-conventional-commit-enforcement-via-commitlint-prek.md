...
- [x] **Task 1: Add the `commitlint` commit-msg hook to `.pre-commit-config.yaml` + update `prepare` to install both hook types** (AC: 1, 5)
...
`pnpm install` output — `prepare` ran `prek install -t pre-commit -t commit-msg`:
...
`ls -la /workspace/ralph-bmad/.git/hooks/commit-msg` → `-rwxr-xr-x 1 dev dev 499 Apr 19 21:24` (executable; 499-byte prek-generated shell wrapper, head: `#!/bin/sh` + `# File generated by prek` comment).

Self-verification probe — valid message via prek runner:

```
$ pnpm exec prek run --stage commit-msg --commit-msg-filename /tmp/msg-ok.txt commitlint
commitlint (conventional commits)........................................Passed
valid-exit=0
```

Self-verification probe — invalid message:

```
$ pnpm exec prek run --stage commit-msg --commit-msg-filename /tmp/msg-bad.txt commitlint
...
- **Variance — `prek 0.3.9` requires `--commit-msg-filename <FILE>`, not `--files <FILE>`, for commit-msg-stage probes via `prek run`.** Spec text (Task 1 self-verification probe block + AC 5) used `prek run --stage commit-msg --files /tmp/<msg-file> commitlint`. That invocation fails with `error: the following required arguments were not provided: --commit-msg-filename <COMMIT_MSG_FILENAME>` and exit 2. Prek separates `--files` (file list to lint, used by pre-commit-stage hooks) from `--commit-msg-filename` (the message file path that gets passed as the trailing positional to commit-msg-stage hooks). Use `--commit-msg-filename` for commit-msg-stage manual runs. AC 5 spec text should be re-read with this correction; the underlying parity claim is unchanged (same hook id, same config, same outcomes).
- **Variance — `prek run` requires `.pre-commit-config.yaml` to be staged before it will execute.** Working-tree edits aren't respected by `prek run`; first attempts after editing the config fail with `error: prek configuration file is not staged, run \`git add .pre-commit-config.yaml\` to stage it` and exit 2. This is a feature (matches what would actually run at git-commit time), not a bug. Resolution: `git add .pre-commit-config.yaml` before invoking `prek run` for self-verification.
