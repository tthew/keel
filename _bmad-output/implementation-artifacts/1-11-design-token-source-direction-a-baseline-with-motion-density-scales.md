...
- [ ] **Task 3: Register `INV-tokens-source` in the invariants manifest + `INVARIANTS.md`** (AC: 4)
  ...
  - [ ] Append one entry to `packages/keel-invariants/src/invariants.manifest.ts` → `raw` array (after the last entry `INV-tokens-semantic-rationale` at line 137; maintain trailing comma + array closure):
    ```typescript
    {
