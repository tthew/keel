...
- [ ] [Review][Patch] **AI-1 — `clean.sh` interactive `read` aborts under `set -euo pipefail` when stdin is non-TTY/EOF** [`packages/devbox/scripts/clean.sh:105`]
...
- [ ] [Review][Patch] **AI-2 — `start.sh` exit 11 on `exited|dead|removing|paused` breaks the documented "container left running for debugging" guarantee** [`packages/devbox/scripts/start.sh:84-87`]
...
- [ ] [Review][Patch] **AI-3 — `monitor-host.sh` exit-3 hint promises a remediation path that doesn't fix the failure mode** [`packages/devbox/scripts/monitor-host.sh:41`]
...
- [ ] [Review][Patch] **AI-4 — `clean.sh` backend detection: `/.dockerenv` over-classifies as B AND inspect-failure defaults to A (less protective)** [`packages/devbox/scripts/clean.sh:80-88`]
