...
- [ ] **Task 6 — Amend RALPH.md execute-spine documentation.** (AC: 3)
  - [ ] Subtask 6.1: RALPH.md is a large file (~52K tokens per create-story Read attempt — exceeds 25K Read limit). Use Grep with targeted patterns + Read (offset/limit) to locate the FR14i references. **Pinned grep tokens** (re-run at dev-story to get live line numbers; create-story line refs may have drifted):
...
- [ ] **Task 9 — Final verification.** (AC: 1, 2, 3, 4, 5, 6)
  ...
  - [ ] Subtask 9.3: `uv run pytest` — full GREEN (Story 1.18's Python smoke corpus; should be unaffected by Story 1.20). Same pre-existing-failure carve-out as Subtask 9.2.
