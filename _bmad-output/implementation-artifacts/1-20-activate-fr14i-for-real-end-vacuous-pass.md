...
CR-pass-1 via `/bmad-code-review (args: "2")` at iter-397. Three parallel review layers: Blind Hunter (diff-only adversarial), Edge Case Hunter (diff + project read), Acceptance Auditor (diff + spec — all 6 ACs PASS). Triage: **0 patch, 3 defer, 16 dismissed** (forecast envelope was 0–3 PATCH per iter-362/369 substrate-extension class CR baseline — landed at lower edge; 6th consecutive substrate-extension class clean-gate datapoint, 1st CR-pass-1 0-PATCH datapoint of class).

- [x] [Review][Defer] Whole-file sha256 fragile to CRLF / whitespace / trailing-newline mutations [`packages/keel-invariants/src/manifest-reader.ts:13-19`; entry at `packages/keel-invariants/src/invariants.manifest.ts:443`] — deferred, pre-existing substrate-wide pattern (affects every whole-file `contentHash` entry in the manifest, not Story 1.20 specific). Carry-to: Story 1.21 audit (consider `.gitattributes text eol=lf` enforcement OR `hashScope: 'normalized-utf8'` canonicalization layer).
- [x] [Review][Defer] Whole-file sha256 catches unintentional drift but does NOT catch adversarial semantic regression of the AC5 trigger-filter clause [`packages/keel-invariants/src/invariants.manifest.ts:438-445`] — deferred, by-design at AC1 lock (whole-file sha256 IS the contract). A future edit could remove `'feat/epic-*'` from the filter and re-bump the manifest hash in lockstep, satisfying the gate while regressing intent. Carry-to: Story 1.21 audit (semantic-content-protection class — consider `hashScope: { yamlPath: 'on.pull_request.branches' }` clause-level lock OR a dedicated AC5-clause guard test).
- [x] [Review][Defer] Inherited `INV-package-test-coverage-floor` content-hash-mismatch deferred without git-log root-cause investigation [`_bmad-output/implementation-artifacts/deferred-work.md` § Story 1.20 dev-story (2026-04-26)] — deferred, pre-existing on `feat/epic-2-packaged-devbox` HEAD pre-Story-1.20-edits per Subtask 9.2 inherited-failure carve-out (consistent with Story 1.18 SC-3 / Story 1.19 SC-9 inherited-failure pattern). Already in deferred-work.md § Story 1.20 dev-story carve-out; CR audit reinforces that Story 1.21 audit scope must include `git log` trace of `check-package-test-coverage-floor.ts` to determine whether the file edit was intentional (and revert if not) before re-bumping contentHash. Carry-to: Story 1.21 audit.

Dismiss tally (16): Trigger filter glob `feat/epic-*` doesn't cross `/` (matches AC5 spec verbatim); glob matches `feat/epic-` literal trailing dash (no operational consequence); `push.branches` vs `pull_request.branches` reference-frame asymmetry (matches design intent — fire on PR + post-merge to base, not on every working-branch push; iter-394 GREEN datapoint empirically validates); smoke test asserts contentHash format only not file-match (locked at Subtask 8.1 negative assertion — no `runSyncGate`); AC3 strict-text contract not literally satisfied at orient/execute step bullets (Subtask 6.1 grep-token surface adaptation pre-blessed at SM-validate iter-388 + SM-verify iter-396; intent met via Signposts/Decisions placement); smoke test doesn't cover AC2 drift behavior with new id (AC2 narrowing rationale — generic mechanic inherited from Story 1.19); smoke test imports `.js` extension assumes build emitted `dist/` (codebase convention; turbo widen fix iter-393 covers); AC3 third clause references PRD line 959 (preservation-only AC; no PRD edit in Story 1.20); manifest description embeds `feat/epic-*` literal (doc-rot risk inherent to all manifest descriptions, not Story 1.20 specific); CI workflow trigger doesn't fire on first push to base until merged (standard GitHub Actions behavior; iter-394 GREEN datapoint confirms PR push DID fire post-Story-1.20 land); no YAML validation on edited workflow (visual inspection sufficient for 4-line edit; iter-394 GREEN empirically validates); whole-file sha256 lacks semantic awareness (re-stated by F4 + already deferred); lockstep contentHash refresh via `replaceAll` (process risk, not code defect; sha256 collision astronomical); sprint-status.yaml duplicate `last_updated:` keys (pre-existing, broken-by-design across all stories); AC6 option-b-defer rationale "circular" (worktree env constraint per AGENTS.md § Worktrees is empirical, not assumption; option-a-resolve requires non-worktree clone access blocked); AC4 cross-reference at section-prose vs bullet-level (AC wording ambiguous; section-level satisfies natural reading); AC6 4th deferred drift outside 3-drift scope (already documented as inherited-failure carve-out per Subtask 9.2; "not a violation" per auditor verdict).
...
- v1.4 — 2026-04-26 — `/bmad-code-review (args: "2")` CR-pass-1 at iter-397. FR14n `sm-verified → done`. **0 PATCH applied** (lower-edge of forecast envelope 0–3 PATCH per iter-362/369 substrate-extension class CR baseline; 6th consecutive substrate-extension class clean-gate datapoint; **1st CR-pass-1 0-PATCH datapoint of class** — Story 1.17 CR-pass-1 iter-362 + Story 1.18 CR-pass-1 iter-369 + Story 1.19 CR-pass-1 iter-377 all had PATCH > 0 at first CR pass). Three parallel review layers: Blind Hunter (diff-only adversarial), Edge Case Hunter (diff + project read), Acceptance Auditor (diff + spec — all 6 ACs PASS). Triage: 0 patch + 3 defer + 16 dismiss. The 3 deferred items all carry-to Story 1.21 audit (whole-file sha256 CRLF/whitespace fragility — substrate-wide; whole-file sha256 lacks semantic-clause awareness — design-class consideration; inherited `INV-package-test-coverage-floor` content-hash-mismatch root-cause investigation — already in deferred-work.md § Story 1.20 dev-story carve-out, CR audit reinforces investigation scope). 16 dismissals split across: design-intent matches (`push.branches` reference frame, AC5 glob spec-verbatim), pre-blessed locks (Subtask 8.1 no-runSyncGate, Subtask 6.1 grep-token surface adaptation, AC2 narrowing rationale), substrate-wide pre-existing patterns not Story-1.20-introduced (CRLF concerns are universal; sprint-status duplicate keys; description doc-rot risk), empirically-validated concerns (iter-394 GREEN datapoint sunsets the trigger-on-first-push concern + the no-YAML-validation concern), and AC-wording-interpretation questions (AC3 placement-shape divergence pre-blessed at SM-validate iter-388 + SM-verify iter-396; AC4 cross-reference at section-prose level satisfies natural reading). Cumulative pre-merge PATCH Story 1.20 lifecycle FINAL: **11** (UNCHANGED from iter-388 SM-validate through iter-397 CR-pass-1; tracking lower edge of forecast envelope 8–18). Story 1.20 DONE — Epic 1 REOPEN-ARC 4 of 5 stories DONE (1.17 + 1.18 + 1.19 + 1.20; remaining: 1.21). Status: `review → done`. Next NOW per FR14n matrix `done` row + IP QUEUE: `Run /bmad-create-story` for Story 1.21 (audit + sweep — picks up the 3 CR-pass-1 defers + Story 1.19 CR-pass-2 defers + Story 1.20 dev-story 4-drift defers + iter-391 devbox-network whitelist gap).
