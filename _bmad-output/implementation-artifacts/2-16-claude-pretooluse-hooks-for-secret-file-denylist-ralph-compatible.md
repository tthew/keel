...
Status: ready-for-dev <!-- Ralph-internal `Story State` = `_(no story) → drafted` at iter-302 `/bmad-create-story` LANDING per § Story Lifecycle row `_(no story) → drafted`. Sprint-status row flipped `2-16-…: backlog → ready-for-dev` at Step 6. Story 2.16 is the Claude PreToolUse hook script + `.claude/settings.json` `hooks` block registration + `INV-claude-hook-secret-denylist` manifest entry (35th — first manifest-adding story since Story 2.14 33→34) + N=3 halt-threshold pin + `.ralph/config.toml` schema + JSONL schema for `blocked-tool-calls.jsonl` (Epic 4 FR37 consumer) + AC 6 retroactive completion of Story 2.15 forward-ref. Forecast: substantive net-new surface — 7 Tasks (hook script + settings.json hooks block + invariant doc + manifest + INVARIANTS.md anchor + .ralph/config.toml + AGENTS.md H3 + CLAUDE.md bullet + packages/devbox/README.md H2 + 2 seed files + keel-templates README). Forecast band 6-10 cumulative lifecycle PATCH per iter-286 NOVEL LESSON narrow-diff-moderate-novelty baseline + iter-301 NOVEL LESSON CANDIDATE +3 if doc-heavy CR drift generalises (Story 2.16 touches 4 docs + 2 JSON files + 1 bash script + 1 TOML + 1 invariant doc — wider doc surface than Story 2.15). Next iter (iter-303): `/bmad-create-story (args: "review")` pre-dev SM per § Story Lifecycle row `drafted → validated`. -->
...
- [ ] **Task 8: `packages/keel-templates/` seeds + README seeded-asset bullets (substrate-to-seed lockstep)** (AC 1, AC 2, AC 7 — Epic 15a Story 15a.4 consumer)
  ...
  - [ ] **Sub-task 8c: `packages/keel-templates/README.md` seeded-asset bullet append.** The existing `## Seeded assets` H2 (landed iter-298 Task 6) currently has one bullet (Story 2.15 settings.json). Story 2.16 APPENDS sibling bullets — DO NOT touch the existing bullet:
    ...
    ```
    The Story 2.15 settings.json bullet stays as-is (byte-identical to substrate; substrate now carries the `hooks` block, so the seed-bullet's content description should mention that the seed includes BOTH `permissions` (Story 2.15) AND `hooks` (Story 2.16)). Optionally amend the existing bullet to the form: "Story 2.15+2.16 committed Claude Code permissions + PreToolUse hook registration baseline (deny/allow + hooks.PreToolUse per NFR5a/5b)" — but if Story 2.15's bullet wording is preserved verbatim, append a clarifying parenthetical. Decision: amend the existing bullet to include the `hooks` block reference (the bullet is the consumer-facing description of what the seed contains; describing it as "deny/allow only" would now be inaccurate). SC-15 sibling-append discipline applies to the SECTION (don't modify `# @keel/keel-templates` H1 or other sections); a single-bullet AMEND inside the Seeded assets H2 is consistent with the consumer-contract intent.
  - [ ] **Verification gates:**
...
- **AMEND path** — if a fork needs a substrate-wide change (e.g. "all forks should remove a substrate-deny rule that causes false positives"), open a PR with 5-site lockstep update: (a) substrate `.claude/hooks/block-secret-access.sh`, (b) substrate `.claude/settings.json` `hooks.PreToolUse` block (if matcher set changes), (c) `docs/invariants/claude-hook-denylist.md`, (d) `packages/keel-invariants/src/invariants.manifest.ts` contentHash, (e) `INVARIANTS.md` H3 anchor + bullet, (f) `packages/keel-templates/src/seeds/.claude/hooks/block-secret-access.sh` seed, (g) `packages/keel-templates/src/seeds/.claude/settings.json` seed. (Substrate-internal cross-refs at AGENTS.md H3 + CLAUDE.md bullet + packages/devbox/README.md H2 may also need updates if the change is operator-visible.) Substrate-wins precedence per `docs/invariants/fork.md § Precedence`.
