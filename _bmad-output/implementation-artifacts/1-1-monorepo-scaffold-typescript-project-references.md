...
- [x] **Task 7: Verify `pnpm -w typecheck` green + Turborepo cache hit** (AC: 3)
  - [x] Run `pnpm -w typecheck` — all packages type-check, exit 0.
...
**Task 7 — `pnpm -w typecheck` × 2 with turbo cache verification (2026-04-19):**

Initial run surfaced `TS5090: Non-relative paths are not allowed when 'baseUrl' is not set` at every per-package `tsconfig.json` — the `paths` map inherited from `tsconfig.base.json` used bare values like `"packages/db/src/index.ts"`. TS 5.0+ requires either a `baseUrl` or a leading `./` on every path value. Fix: prefixed all 15 `paths` values in `tsconfig.base.json` with `./` (e.g., `"./packages/db/src/index.ts"`). Chose the relative-path fix over setting `baseUrl: "."` to keep bare-import resolution (via `moduleResolution: bundler` + package.json) independent of the paths map.
...
**Modified (Task 7):** `tsconfig.base.json` — prefixed each `paths` value with `./` to satisfy TS5090 (non-relative paths require `baseUrl`).
