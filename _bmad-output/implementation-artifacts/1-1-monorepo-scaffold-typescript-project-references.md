...
- [ ] **Task 6: Verify `pnpm install` green** (AC: 1)
  - [ ] Run `pnpm install` from repo root. Must complete with exit 0 and no warnings about missing workspace members.
  - [ ] `pnpm -r list --depth -1 --json` should enumerate all 15 workspace members (14 packages + `apps/web`). Capture as evidence in the dev record.
...
_To be filled by dev-story iteration. On completion, explicitly note: (a) pnpm + TypeScript + Turbo exact versions used; (b) `pnpm -w typecheck` cache-hit evidence; (c) the `apps/web` TanStack scaffold gap flagged to PM for course-correction._
