...
**Given** Stories 1.18 + 1.19 (CR-pass-1 + CR-pass-2) + 1.20 (dev-story + CR-pass-1) accumulated 19 deferred entries during the Epic 1 REOPEN-ARC carrying-`Carry-to: Story 1.21 audit`-rationale (4 from Story 1.18 CR; 7 from Story 1.19 CR-pass-1; 5 from Story 1.19 CR-pass-2; 4 from Story 1.20 dev-story; 3 from Story 1.20 CR-pass-1 — exact count to be ground-truthed at dev-story open via grep against `deferred-work.md` § headers carrying `(2026-04-26)` or `(2026-04-27)` dates),
...
- [ ] **Task 1 — Pre-flight ground-truth substrate probe.** (AC: 1, 3, 5 — precondition)
  ...
  - [ ] Subtask 1.2: Walk `_bmad-output/implementation-artifacts/deferred-work.md` for inherited-DEFER scope ground-truth. Capture the EXACT entry count under each of the 5 Epic-1-REOPEN-ARC sections (`code review of story-1-18 (2026-04-26)`, `code review of story-1-19 (2026-04-26)`, `code review of story-1-19 CR-pass-2 (2026-04-26)`, `Story 1.20 dev-story (2026-04-26)`, `code review of story-1-20 (2026-04-26)`). Predicted: 4 + 7 + 5 + 4 + 3 = 23 entries (revised upward from IP § Notes "~19" — re-grep to ground-truth at dev-story open). The exact list drives AC5's per-DEFER disposition decisions.
...
The Story 1.21 audit walks **30 stories** total (Epic 1: 1.1–1.16 = 16 stories; Epic 2: 2.1–2.18 = 18 stories; Stories 1.17–1.20 are EXCLUDED — they are the bootstrap arc that landed AFTER the FR14n amendment per issue #233; their ATDD posture is post-amendment, not pre-bootstrap-grandfathered).
...
- **Story 1.19 CR-pass-2** (5 entries): CR-1/CR-2 red-path tests use loose `expect.stringMatching(...)`; CR-6 only exercises absent-`hashScope` branch; `HAS_BASH_DASH` guard; EPIPE handling; CR-3 stdout-empty assertion; CR-8 JSON.parse SyntaxError diagnostic.
...
Total predicted: **23 inherited DEFERs**. Subtask 1.2 ground-truths the actual count + drives the per-disposition checklist (AC5 lock).
...
- Files **modified**:
  ...
  - `_bmad-output/implementation-artifacts/sprint-status.yaml` (Subtask 11 of `bmad-create-story` workflow flips `1-21-...: backlog → ready-for-dev`; subsequent state transitions per FR14n matrix).
