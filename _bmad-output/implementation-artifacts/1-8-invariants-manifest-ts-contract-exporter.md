...
_Code review iter-7 (2026-04-20; Ralph-hosted `/bmad-code-review (args: "2")`; three parallel layers — Blind Hunter + Edge Case Hunter + Acceptance Auditor). Triage: 0 decision-needed, 0 patches, 6 deferred, 19 dismissed. **Acceptance Auditor returned zero findings** (all 4 ACs satisfied; all 9 sha256 `contentHash` values re-computed on disk and verified matching; `id`-regex conformance, sourcePath existence, Zod top-level `parse` all confirmed)._

- [x] [Review][Defer] contentHash drift not validated at manifest-use time [packages/keel-invariants/src/invariants.manifest.ts] — Story 1.9 sync-gate scope per FR43 enforcement side + § ContentHash anchor-bounding carve-out; manifest stores the pre-computed hash, Story 1.9's walker does the live-file re-read + diff.
- [x] [Review][Defer] sourcePath accepts `..` / absolute / backslash — no traversal guard in Zod schema [packages/keel-invariants/src/invariants.manifest.ts:6] — Story 1.9 walker owns file-resolution + existence checking; tightening the Zod string regex now is out-of-scope for contract-only Story 1.8.
- [x] [Review][Defer] `id` uniqueness across array not enforced by Zod (`.superRefine` / Set check) [packages/keel-invariants/src/invariants.manifest.ts:11] — Story 1.9 drift detection ("unregistered rules as drift" per AC 3 enforcement side) catches duplicates end-to-end; auditor confirmed current 9 IDs are unique.
- [x] [Review][Defer] Shared-sourcePath cross-entry `contentHash` consistency not refine-checked in schema [packages/keel-invariants/src/invariants.manifest.ts:11] — Story 1.9 walker re-computes `sha256` per distinct sourcePath and surfaces mismatch; current 4 shared-source pairs (eslint.config + .pre-commit-config) are verified consistent.
- [x] [Review][Defer] `invariants` array is mutable — no `readonly Invariant[]` / `Object.freeze` [packages/keel-invariants/src/invariants.manifest.ts:89] — spec-silent; defensive hardening candidate for Story 1.9 refactor or a follow-up substrate-hardening story.
- [x] [Review][Defer] No `schemaVersion` / `deprecated` / `since` metadata on `Invariant` entries [packages/keel-invariants/src/invariants.manifest.ts:3-9] — spec-silent; evolution-strategy metadata belongs with Story 1.9's enforcement side (which will need "deprecated ≠ drift" semantics) or a later hardening story.

_19 additional findings dismissed as by-design (AC-mandated Zod import-time throw, intentional whole-file contentHash sharing, deliberate Zod-schema re-export for Story 1.9 consumption, deliberate exact-version zod pin per I7 posture), out-of-scope (zero tests — Story 1.16 delivers test runner), or false-positives (TS `Invariant = z.infer<>` can't drift from its own schema; lowercase-only hash regex matches sha256sum output; NodeNext `.js` extension verified green by Task 3 typecheck+build). Adversarial-triage report pinned at `_bmad-output/implementation-artifacts/deferred-work.md § Deferred from: code review of story-1.8 (2026-04-20)`._
...
| 2026-04-20 | 1.4     | Code review (Ralph, iter-7; sm-verified → done per FR14n matrix row 9). Three parallel adversarial layers via `/bmad-code-review (args: "2")`: Blind Hunter (16 findings), Edge Case Hunter (15 findings), Acceptance Auditor (**zero AC violations** — all 9 sha256 contentHashes re-computed on disk and verified matching, 9/9 sourcePaths exist, `id`-regex conformance 9/9, Zod top-level `parse` synchronous). Triage (`step-03-triage.md`): **0 decision-needed, 0 patches, 6 defers, 19 dismissed.** Findings written to § Review Findings above; the 6 defers carried to `_bmad-output/implementation-artifacts/deferred-work.md § Deferred from: code review of story-1.8 (2026-04-20)`. All 6 defers are Story 1.9-enforcement-side scope (contentHash drift validation, sourcePath traversal guard, id uniqueness refine, cross-entry hash consistency) or spec-silent hardening (readonly/freeze, schema-evolution metadata) — none blocking for Story 1.8's contract-only remit. Story 1.8 complete; sprint-status already `1-8: done` (co-landed in iter-4 dev-story commit). | Ralph |
