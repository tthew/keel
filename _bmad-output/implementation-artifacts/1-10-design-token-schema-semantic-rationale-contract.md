...
2. **Given** `docs/invariants/tokens.md`,
   ...
   **And** each rationale carries a stable `TOKEN-<slug>` ID (e.g. `TOKEN-surface-raised`, `TOKEN-status-success`, `TOKEN-motion-scale-snap`).
...
- [ ] **Task 4: Quality gates + sync-gate smoke + sprint-status bump** (no AC — substrate verification)
  ...
  - [ ] Bump `_bmad-output/implementation-artifacts/sprint-status.yaml`: `1-10-design-token-schema-semantic-rationale-contract: backlog → in-progress` at `/bmad-create-story` time, then `in-progress → review` at `/bmad-dev-story` completion. `last_updated: 2026-04-20 Story-1-10-review UTC` (or current date). Preserve all existing comments + structure + STATUS DEFINITIONS block.
  - [ ] Flip this story file's Status to `review` at `/bmad-dev-story` completion (per dev-story workflow step 9; FR14n `atdd-scaffolded → in-dev`). All Task subtasks `[x]`. `## Dev Agent Record` populated with File List + Completion Notes.
