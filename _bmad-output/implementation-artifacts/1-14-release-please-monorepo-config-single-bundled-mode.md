...
- [ ] **Task 1: Author `.github/release-please-config.json` (single-bundled mode; all 16 workspace members + root component = 17 `packages:` entries)** (AC: 1, 2, 3, 4, 5)
...
- [ ] **Task 2: Author `.github/.release-please-manifest.json` (initial versions = 0.0.0 for every workspace package + root component)** (AC: 1)
  - [ ] Author the JSON file at `.github/.release-please-manifest.json`. Shape (all 17 keys; values match current `package.json:version` of each workspace member — which is `0.0.0` everywhere per Task 1 enumeration check):
    ```json
    {
      ".": "0.0.0",
      "apps/web": "0.0.0",
    ...
    ```
  - [ ] Cross-verify every key matches a `packages:` key in `release-please-config.json` from Task 1 (17-key parity — the linked-versions plugin requires this parity on every run; missing-key drift would fail the first real release-please action invocation once Story 13.5 lands).
