...
Status: review <!-- Ralph-internal `Story State` = `in-dev` at iter-283 `/bmad-dev-story` landing (atdd-scaffolded iter-282 → in-dev iter-283). All 5 Tasks complete in a single iteration (Tasks 1-4 mandatory + Task 5 optional D-1 absorption). Sprint-status row flipped `ready-for-dev → in-progress` at Step 4 workflow entry, then `in-progress → review` at Step 9 completion. Next iter (iter-284): `/bmad-testarch-trace (args: "yolo")` per § Story Lifecycle row `in-dev → traced` — forecast WAIVED per Story 2.12 iter-269 pattern (AC 1, 2, 3, 5 substrate-covered by docker-compose-config-based shape smoke + dash -n POSIX parse + sync-gate; AC 4 operator-workstation-deferred). -->
...
- **Task 2 (probe tooling verification):** `dnsutils` (providing `dig`) present at `Dockerfile:61`; `netcat-openbsd` (providing `nc -z`) present at `Dockerfile:64`; `USER dev` at `Dockerfile:347`; `sshd_config` has no `ListenAddress` directive (PATCH-4 verification PASS — OpenSSH unset-default listens on IPv4 `0.0.0.0` + IPv6 `::`, so `nc -z 127.0.0.1 2222` IPv4 probe connects). No Dockerfile change needed for Task 2.
...
**Created (1):**
- `docs/invariants/devbox-healthcheck.md` — new machine-enforced invariant doc (sha256 `665174a3592106eabbb9f6b5dd39ce813839b3c61a89f1709d81e90524d18623`; 7 H2 contract sections + § Intent).
