...
**Given** the CLAUDE.md `## Common commands` table at lines 11–22 (pre-edit; post-landing footer extends to line 25 — the three new rows added by Task 8),
...
- [x] **Task 8 — Update CLAUDE.md `## Common commands` table with `pnpm test` row.** (AC: 5)
  - [x] Subtask 8.1: Edit `CLAUDE.md` `## Common commands` table at lines 15–22 (pre-edit shape per substrate verification at SM-validate; post-landing footer extends through line 25 with the three new rows): the table has no separate footer row — line 22 (`| Stop the Ralph loop | …`) WAS the last table row pre-edit, with prose at line 23+. Append the new row `| Run all tests | \`pnpm test\` |` immediately after line 22, preserving the blank line before the prose paragraph (which sits at line 27 post-landing after the three new rows shift it from line 24). Maintain alignment via prettier-friendly spacing.
...
- **SC-10: AGENTS.md / CLAUDE.md update strategy.** CLAUDE.md table edit is required (AC5). AGENTS.md placement locked at SM-validate: insert a new `## Testing` top-level section between `## How to work here` (ends line 41) and `## Project conventions` (was line 43 pre-edit; now line 47 post-landing — `## Testing` occupies line 43). Substrate probe confirms AGENTS.md has no top-level commands table, so a dedicated short section is the cleanest insertion point. Avoid duplicating prose between the two files; the AGENTS.md form is a brief pointer that says "Run `pnpm test` (Vitest, workspace-wide); `uv run pytest` arrives in Story 1.18" + cross-references `architecture.md § M0` for the canonical rationale.
...
**Empirical-vs-forecast deltas (filed at iter-361 post-dev SM-verify):** Pre-dev SM-validate landed 16 PATCHes (vs forecast 1–2; +14 over band). Root cause: course-correction-author origin (planner had no substrate visibility) + wider-scope-than-2.18 surface — consistent with RALPH.md iter-348 / iter-357 pattern. Subsequent gates matched forecast: ATDD-skip (0 PATCH), dev-story (0 PATCH single-iter clean landing per substrate-extension class iter-344), trace WAIVED (0 PATCH per structural-artefact rationale), post-dev SM-verify ≈ 4 PATCHes (within 1–4 envelope per RALPH.md iter-352 narrow-substrate-extension band — 2 MUST-FIX line-anchor staleness + 2 SHOULD-FIX line-range / forecast-empirical-delta). Cumulative lifecycle pre-merge PATCH count tracking ~20 (vs forecast band 2–11) — over band but explainable by course-correction-author origin compounding at the SM-validate gate; post-dev gates well-bounded.
...
| CLAUDE.md `## Common commands` table | `CLAUDE.md:11-22` (pre-edit; post-landing :11-25) | EXISTS (6 rows pre-edit; shape verified at SM-validate. Post-landing: 9 rows after Task 8 added `pnpm test` / `pnpm typecheck` / `pnpm lint`) | none — story extends |
...
- `.github/workflows/ci.yml` — first CI workflow (single `node` job; `pnpm install --frozen-lockfile` + `pnpm turbo run test lint typecheck`; pnpm 10.29.2 + Node 20 pinned per SC-11). **CR iter-362 amendment:** added top-level `permissions: contents: read` block + `concurrency: { group: ci-${{ github.ref }}, cancel-in-progress: true }` block per `/bmad-code-review` inline-bundle-close (least-privilege GITHUB_TOKEN scope + duplicate-run cancellation).
...
- **v1.5** (2026-04-25, iter-361) — Post-dev SM-verify via `/bmad-create-story (args: "review")`. FR14n state transition `traced → sm-verified`. Two-subagent review (technical-correctness + prose-density) per RALPH.md iter-235 narrow-surface pattern + iter-352 post-dev SM-verify recipe. **Subagent A (technical-correctness): GREEN** — every AC verified against landed substrate (vitest 3.2.4 pinned at all 4 sites; smoke test passes; `INV-prek-prepare-lifecycle` contentHash starts `74237244` as claimed; 3 pre-existing `INV-git-hooks-preservation` drifts confirmed; CI workflow step ordering matches Subtask 7.2 exactly; File List complete vs `git show --stat f7aad91`). 2 MUST-FIX (Change Log block-reorder; Subtask 9.1 stale line-anchors) + 3 SHOULD-FIX (line-range drift on CLAUDE.md table citations: AC5 + ledger row + Subtask 8.1). **Subagent B (prose-density): YELLOW-LEAN-GREEN** — convergent on the 2 MUST-FIX, plus 5 additional SHOULD-FIX (SC-9 dangling, Subtask 1.1 historical, ledger row 10 stale, References dead-weight, Forecast empirical divergence, Completion Note redundancy) + 1 NIT + 1 LLM-OPT. **4 PATCHes applied inline at gate** (within RALPH.md iter-352 forecast envelope 1–4): (1) MUST-FIX M1 — Change Log entries reordered to chronological v1.0/v1.1/v1.2/v1.3/v1.4; (2) MUST-FIX M2 — Subtask 9.1 + SC-10 annotated with pre-edit / post-landing line anchors (`## Project conventions` was line 43 → now line 47); (3) SHOULD-FIX bundle (A's line-range drift cluster) — AC5 + Substrate ledger row + Subtask 8.1 annotated with pre/post-edit table footer (`:11-22` → `:11-25` post-landing); (4) SHOULD-FIX (B's S5) — appended empirical-vs-forecast deltas block to § Forecast envelope (SM-validate 16 actual vs 1–2 forecast; subsequent gates matched). Deferred: 6 SHOULD-FIX (SC-9 fold, Subtask 1.1 historical-collapse, ledger row 10 cleanup, References trim, Completion Note redundancy) + 1 NIT + 1 LLM-OPT — all cleanup-class, not blocking. **Post-dev SM-verify yield empirical (course-correction-author origin, wider-scope-than-2.18, post-clean-dev-landing):** 4 PATCH at gate vs RALPH.md iter-352 baseline of 5 PATCH — narrow-band consistent with iter-281 density-amortisation prediction (impl-landing absorbs technical-correctness drift, leaving line-range drift + forecast-historical residuals). Story file `Status: review` unchanged; sprint-status row unchanged (SM-verify is Ralph-internal per FR14n). 0 fix-task QUEUE entries → direct promotion to `sm-verified`. Cumulative pre-merge PATCH count Story 1.17 lifecycle to date: 20 (16 + 4). Next NOW = `/bmad-code-review (args: "2")` (`sm-verified → done` or `fixes-pending`); forecast 0–2 PATCH inline-bundle-close per RALPH.md iter-342 mid-arc narrow-band recipe.
- **v1.6** (2026-04-25, iter-362) — Code review via `/bmad-code-review (args: "2")` Ralph-canonical realisation (three parallel `general-purpose` Agent calls — Blind Hunter + Edge Case Hunter + Acceptance Auditor — per RALPH.md iter-294; diff-surface enumerated upfront in EVERY subagent prompt per iter-308). FR14n state transition `sm-verified → done`. **Acceptance Auditor: PASS** — 5/5 ACs SATISFIED; 11/11 SCs SATISFIED; Tasks 1:1 with `git show --stat f7aad91`; 0 cross-spec drift; 0 MUST-FIX; 0 SHOULD-FIX above threshold. **Blind Hunter** (2 CRITICAL / 5 HIGH / 6 MEDIUM / 4 LOW / 3 NIT) + **Edge Case Hunter** (1 CRITICAL / 3 HIGH / 4 MEDIUM / 4 LOW / 3 NIT) findings triaged per RALPH.md iter-138 bundling + iter-342 inline-bundle-close conditions. **2 PATCHes applied inline at gate** (within forecast envelope 0–2; both targeting `.github/workflows/ci.yml` — single surface bundled per iter-138): (1) HIGH (Blind/Edge convergent) — added top-level `permissions: contents: read` block (least-privilege GITHUB_TOKEN scope; defends against malicious-dep postinstall-hook attack surface during `pnpm install`); (2) HIGH (Blind/Edge convergent) — added `concurrency: { group: ci-${{ github.ref }}, cancel-in-progress: true }` block (cancels duplicate runs on rapid pushes). **DEFERRED to Story 1.19/1.20/1.21 followups** (cleanup-class, spec-pinned, or non-blocking): legacy `node:test` orphan (CRITICAL Edge / Blind — spec SC-3 explicit deferral to Story 1.19; comment in `vitest.config.ts` records it); `defineWorkspace` 3.2 deprecation path (CRITICAL Blind — Story 1.21 sweep); third-party actions SHA-pinning (HIGH Blind — Story 1.20/1.21); Vitest exclude defaults spread (HIGH Blind — Story 1.21); smoke-test signal strength + coverage wiring (MEDIUM Blind/Edge — Story 1.19 backfill per spec SC-1, SC-7); Node version matrix / OS matrix / job naming / reporter (LOW Blind/Edge — Story 1.21 audit). **DISMISSED:** `pnpm turbo run test lint typecheck` arg order (HIGH Blind — turbo schedules topologically, not by arg order); `pull_request: branches: [main]` filter (HIGH Edge — spec AC3 + SC-6 explicit); `pnpm.overrides` + per-package devDep "redundancy" (HIGH Blind — spec SC-9 documents I7 dual-site pattern); hash refresh recomputation evidence (MEDIUM Blind — sync-gate IS the verification mechanism); smoke-test weak assertion (MEDIUM Blind/Edge — spec SC-1 + SC-4 verbatim); `outputs: ["coverage/**"]` without coverage producer (MEDIUM Blind/Edge — spec SC-7 explicit forward-compat). **5th precedent of inline-bundle-close pattern** (Stories 2.15/2.16/2.17/2.18/1.17). **CR yield empirical (course-correction-author origin, mid-arc not end-of-epic):** 2 PATCH at CR vs RALPH.md iter-353 Story 2.18 baseline of 2 PATCH (1 CRITICAL `whitelist.sh cmd_sync` + 1 MEDIUM downstream-reference debt) — same count, different class: Story 1.17's 2 PATCHes are workflow-hardening additions (not in-spec) vs Story 2.18's 2 PATCHes were correctness-bug + downstream-debt residuals; this confirms RALPH.md iter-301 doc-heavy-narrow-diff drift class is real BUT Story 1.17's diff is config-heavy not doc-heavy, so the CR class shifts to "workflow-security-hardening" (a NEW residual class for this story-type — first CI workflow ever shipped in repo). Story file `Status: review → done`; sprint-status row `1-17-…: review → done`. 0 fix-task QUEUE entries → direct promotion. Cumulative pre-merge PATCH count Story 1.17 lifecycle to date: 22 (16 SM-validate + 4 SM-verify + 2 CR; 0 dev-story; 0 trace). Next iter NOW = `/bmad-create-story` for Story 1.18 (Python pytest under uv + root pyproject.toml).
