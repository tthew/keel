...
- [ ] **Task 5 — `check-claude-hook-syntax.ts` integration test.** (AC: 2)
  ...
  - [ ] Subtask 5.3: Skip-on-missing-bash/dash guard: at the top of the test file, check `which bash && which dash` availability; if either missing, mark the affected tests `it.skip(...)` with rationale captured in Completion Notes. **Locked at SM-validate iter-371:** cc-devbox iteration env carries both — `which bash` → `/usr/bin/bash`, `which dash` → `/usr/bin/dash` (verified in worktree). The skip-guard is defensive only (covers fork-replacement runtimes that may strip dash); not expected to trigger in the canonical Ralph loop.
...
- [ ] **Task 6 — sync-gate four-drift-class integration test.** (AC: 3)
  ...
  - [ ] Subtask 6.2: Wrinkle — `runSyncGate` reads `invariants` from `manifest-reader.ts` (which re-exports from `invariants.manifest.ts` at line 8 + ALSO carries 119 lines of crypto/fs helper logic per Dev Notes substrate-ledger correction; see § manifest-reader.ts surface). To inject a fixture invariants array, mock the manifest-reader module via vitest. **Locked at SM-validate iter-371: Strategy A — `vi.mock('../manifest-reader.js', ...)` per-test to override the `invariants` export.** No Strategy B pre-authored. If dev-story finds `vi.mock` module-spec brittle (e.g. ESM hoist semantics fail under vitest 3.x's specific config), raise a CR finding; do NOT silently switch to `vi.doMock(...)` + dynamic import. Per RALPH.md iter-357 lock-don't-defer rule.
