...
Status: ready-for-dev <!-- sprint-row status — Ralph-internal `Story State` = `validated` at iter-246 per § Story Lifecycle Decision Matrix row `drafted → validated`; pre-dev SM gate does NOT flip sprint-row state (mirrors trace + post-dev SM gate precedent). -->
