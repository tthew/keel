...
- [ ] **Task 1: Scaffold runtime-infrastructure sibling files under `packages/devbox/`** (AC: 1)
  ...
  - [ ] Create `packages/devbox/scripts/` directory with `.gitkeep` and a README pointing at Story 2.6 as the owner of full CLI wrappers. Story 2.1 does NOT author the lifecycle scripts themselves — only the directory exists.
...
- [ ] **Task 2: Bake substrate toolchain at image-build time** (AC: 2)
  - [ ] In `Dockerfile`, install system dependencies via `apt-get` in a single cached layer: `curl`, `git`, `ca-certificates`, `build-essential`, `unzip`, `jq`, plus Playwright browser OS-level deps (per [playwright.dev: ubuntu 24.04 deps list](https://playwright.dev/docs/browsers#install-browsers) — keep the list inline so Renovate + Story 1.15 can track).
...
- [ ] **Task 4: Verify substrate tooling access from inside the image** (AC: 3)
  ...
  - [ ] Run substrate tests from the container via the raw compose command: `docker compose -f packages/devbox/docker-compose.yml run --rm devbox pnpm test`. Confirm exit code 0 (or, if the test suite is not yet wired in existing packages, confirm `pnpm test` executes without "command not found" — the turbo task either passes or reports 0 runnable tasks).
  - [ ] Run substrate lint from the container: `docker compose -f packages/devbox/docker-compose.yml run --rm devbox pnpm lint`. Same acceptance posture — exit 0 OR empty-task-set (not "command not found").
