...
**And** Story 1.9's sync-gate (`pnpm keel-invariants:check`) does NOT regress on any pre-existing manifest entry attributable to this story's edits (the `INV-deps-version-pinning` row covers TS/pnpm only at this scope; Python-dep coverage of the sync-gate is a Story 1.20 follow-up per SC-8 + AC3 of Story 1.18 spec — see SC-9).
...
**Given** the existing `.github/workflows/ci.yml` (Story 1.17 substrate; lines 1–30 — `name: ci` + `on:` + `permissions:` + `concurrency:` + single `node` job),
...
**And** the `node` job (lines 16–29 pre-edit) is byte-identical post-edit (additive YAML, no rewrite per SC-5)
**And** the workflow YAML is syntactically valid (`actionlint .github/workflows/ci.yml` exits 0; falls back to GH Actions ingestion-side validation post-push if `actionlint` is unavailable in the iter env per Story 1.17 Subtask 10.4 precedent).
...
**When** Story 1.18 lands,
**Then** the section's second sentence is rewritten in active tense to document the now-shipped Python entry point (e.g., "Run `uv run pytest` from the worktree root for the workspace-wide pytest suite covering `ralph.py` + `scripts/` + `packages/devbox/tui/`.")
**And** the cross-reference to `architecture.md § M0 substrate developer-productivity floor` is preserved
**And** prettier idempotence holds (`pnpm format:check` exits 0 against AGENTS.md per Story 1.17 SC-10 precedent).

## Tasks / Subtasks

(Authored at `/bmad-create-story` time per FR14n § Story Lifecycle. Forecast 10 tasks / ~22 subtasks per Story 1.17 narrow-substrate-extension precedent — Python sibling is structurally similar but slightly narrower because there is no equivalent of Story 1.17's `turbo.json` extension, no `pnpm.overrides` lockstep, and no legacy-test-API exclusion. Listed here as scaffolding for the next iteration's `/bmad-create-story (args: "review")` SM-validate gate.)
...
- [ ] **Task 4 — Create `scripts/tests/test_bootstrap_bmad_agents.py` smoke.** (AC: 1)
  - [ ] Subtask 4.1: Create directory `scripts/tests/` (ABSENT pre-edit per substrate ledger). Author `scripts/tests/test_bootstrap_bmad_agents.py`. Because the source filename `scripts/bootstrap-bmad-agents.py` carries hyphens (not a valid Python module identifier), use `importlib.util.spec_from_file_location` to load the module:
    ...
    - Note: `scripts/tests/__init__.py` is NOT required because `pythonpath = [..., "scripts", ...]` from Subtask 1.1 plus pytest's `rootdir`-based import-mode default makes test files discoverable directly. But verify discovery posture in Subtask 9.1 — if pytest 8.x default import-mode (`prepend`) requires `__init__.py` at any of `tests/`, `scripts/tests/`, or `packages/devbox/tui/tests/`, add empty `__init__.py` files at all three sites in dev-story time (record decision in Completion Notes).
...
- [ ] **Task 5 — Create `packages/devbox/tui/tests/test_theme.py` smoke.** (AC: 1)
  - [ ] Subtask 5.1: Create directory `packages/devbox/tui/tests/` (ABSENT pre-edit per substrate ledger). Author `packages/devbox/tui/tests/test_theme.py`:
    - `from tui import theme` (importable because `pythonpath` includes `packages/devbox` per Subtask 1.1 + `packages/devbox/tui/__init__.py:1` exists per substrate ledger making `tui` a proper Python package).
    - One `def test_theme_neutral_500():` asserting `theme.theme.colors.neutral_500 == "oklch(52% 0 0)"` — exact literal verified at substrate ledger via reading `packages/devbox/tui/theme.py:15`. The double `theme.theme` is intentional: the module `tui.theme` exports a top-level binding named `theme` (a `SimpleNamespace` per `theme.py:8`), so the qualified path is `<imported-module>.<exported-binding>.colors.<field>`. Alternative authoring shape `from tui.theme import theme` + `theme.colors.neutral_500` is equivalent; pick at dev-story time (record in Completion Notes).
...
- [ ] **Task 6 — Extend `.github/workflows/ci.yml` with `python` job.** (AC: 3)
  - [ ] Subtask 6.1: Edit `.github/workflows/ci.yml` (lines 1–30 pre-edit per substrate ledger; Story 1.17 substrate). The edit is ADDITIVE per SC-5 — the existing `node` job (lines 17–29) is byte-identical post-edit; a new `python` job is appended at the same `jobs:` indentation level. Pre-edit `jobs:` block ends at line 29 (`- run: pnpm turbo run test lint typecheck`); post-edit, the `python` job follows on a new line after the `node:` job's last step.
  - [ ] Subtask 6.2: `python` job shape: `runs-on: ubuntu-latest`; steps in this exact order — (1) `actions/checkout@v4` (matches Story 1.17's `node` job major-pin per SC-4), (2) `astral-sh/setup-uv@v6` with `version: "0.11.7"` (matches the iter-env `uv` per substrate probe — exact-version pin per I7; OR `latest` if dev-story time prefers the at-CI-resolution path, decided in Completion Notes per SC-4), (3) `uv sync --extra dev --frozen` (uses `uv.lock` deterministically per AC2), (4) `uv run pytest` (the canonical entry point per FR14o). Pin all GitHub Action versions per I7 using `@v4` / `@v6` major-pin for first-party + verified-publisher actions; avoid third-party actions (Story 1.17 CR iter-362 deferred SHA-pinning of third-party actions to Story 1.20/1.21 per its Change Log v1.6).
...
| `pnpm-workspace.yaml` | `pnpm-workspace.yaml:1-4` | EXISTS (`packages: [apps/*, packages/*]`) | none — Python is OUT of pnpm workspace |
| `.gitignore` Python patterns | `.gitignore:61-72` | EXISTS (`__pycache__/`, `*.py[cod]`, `.venv/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`) | none — story relies on existing patterns; no edits |
...
| Architecture § M0 substrate developer-productivity floor | `_bmad-output/planning-artifacts/architecture.md:198-237` | EXISTS verbatim (Python runtime substrate sub-section at lines 210–215; CI integration at 216–223; bootstrap arc at 225–231) | none |
...
| sprint-status row | `_bmad-output/implementation-artifacts/sprint-status.yaml:142` (`1-18-bootstrap-python-test-runner-pytest-under-uv: backlog`) | EXISTS (status `backlog` pre-edit; this story flips to `ready-for-dev` per Subtask 10.1) | none |
...
Optionally created at dev-story time (decided in Subtask 4.1 Completion Notes per pytest 8.x import-mode posture):

- `tests/__init__.py` / `scripts/tests/__init__.py` / `packages/devbox/tui/tests/__init__.py` — empty marker files for pytest collection (only added if `pythonpath` + import-mode default require them).
...
- [Source: `_bmad-output/planning-artifacts/architecture.md:198-237` § M0 substrate developer-productivity floor — Python runtime substrate sub-section + CI integration + bootstrap arc]
