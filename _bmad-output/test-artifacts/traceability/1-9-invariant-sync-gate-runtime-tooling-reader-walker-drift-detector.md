...
- **Substrate verification (non-gate-eligible evidence):**
  ...
  - Exercised via iter-8 Task-5 manifest-side missing-anchor smoke (delete `INV-commitlint-shared` anchor line from `INVARIANTS.md` → exit 1 + `added-to-source-only` with sourcePath; revert; byte-identical restore) per story file line 180. All 10 manifest rows have matching anchors post-Task-2 (which closed the pre-existing `INV-ralph-halt-path-resolution` docs-only drift by adding the 10th manifest entry per story file lines 57–71) — AC-2 anchor-side branch verified by ephemeral `INV-commitlint-shared` anchor deletion in the smoke.
...
- **Gaps:** No runtime smoke triggering `added-to-source-only`; no test-runner-hosted structural test. Schema-level uniqueness refine (load-bearing complement per Fix O iter-33) + CR adversarial pass (Blind Hunter) per § Testing Standards.
...
- **Recommendation:** Accept structural realisation + Task 2 pre-existing drift closure as evidence that the branch works (the original 10th-entry gap was an instance of `removed-from-docs-only` that the spec's Task 2 explicitly closed — if the branch didn't work, Task 2's close would not have produced a clean gate in Task 5's clean-path smoke). CR adversarial pass (Blind Hunter) per § Testing Standards complements the substrate evidence. Story 1.16 can backfill unit coverage. WAIVED.
...
**5 high (P1) gaps** — four (AC-1, AC-2, AC-4, AC-5) are SUBSTRATE_VERIFIED end-to-end via Task 5 smoke tests but remain uncovered by automated **test-runner-hosted** tests; the fifth (AC-3) is structural-only. Each gap's realisation is documented and carries substrate evidence:
