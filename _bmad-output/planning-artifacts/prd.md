...
- **M0.5 Devbox** (3d) — absorb [`cc-devbox`](https://github.com/tthew/cc-devbox) into `packages/devbox/` with five concrete deliverables:
    ...
    - **(b) Compose** — `docker-compose.yml` with resource limits parameterised via `.envrc` (`KEEL_DEVBOX_ARCH`, `KEEL_DEVBOX_CPUS`, `KEEL_DEVBOX_MEMORY_GB`, `KEEL_DEVBOX_SHM_GB`) and Apple-Silicon-M4-Pro defaults; upstream's hardcoded `/Users/tthew/Development` parent-dir mount collapses into a `.envrc`-driven per-fork vs shared-workspace mount contract.
...
- **Base image & architecture.** Ubuntu 24.04 LTS (multi-stage build inherited from upstream `Dockerfile`). Target platform and resource limits parameterised via `.envrc`: `KEEL_DEVBOX_ARCH` (default `linux/arm64`), `KEEL_DEVBOX_CPUS` (default `8`), `KEEL_DEVBOX_MEMORY_GB` (default `12`), `KEEL_DEVBOX_SHM_GB` (default `2`), `KEEL_DEVBOX_NOFILE` (default `65536`). Defaults match upstream's M4-Pro tuning; non-Apple-Silicon hosts can still fork.
...
- **tmpfs policy.** `/tmp` (default 2 GB), `/var/tmp` (default 1 GB), `/workspace/logs` (default 500 MB), `/run` (autoscaled) — all `noexec,nosuid`. Sizes parameterisable via `.envrc` with defaults matching upstream `docker-compose.yml`. See NFR8.
...
- **NFR8**: `/tmp`, `/var/tmp`, and `/workspace/logs` are mounted tmpfs with `noexec` and `nosuid` flags. Sizes default to 2 GB `/tmp`, 1 GB `/var/tmp`, 500 MB `/workspace/logs` (matching upstream cc-devbox), parameterisable via `.envrc`.
