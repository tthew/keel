...
prdClarificationsRaised:
  ...
  - FR14k + NFR33a path-resolution clarification (2026-04-20) - when `ralph.py` is invoked with `--worktree <name>`, `.ralph/halt`, `.ralph/@plan.md`, `.ralph/PROMPT_*.md`, and `.ralph/logs/` are resolved against the worktree path (`.claude/worktrees/<name>/.ralph/`), not the orchestrator's cwd; the orchestrator exports `RALPH_BASE_DIR` (absolute) to the subprocess env so agent + orchestrator address the same directory. Closes the 2026-04-20 Story 1.7 iter-22..28 re-entry cascade root cause. No new FR/NFR; amendment to FR14k + § Agent Workflow Contracts → Halt schema + NFR33a
  - FR14n + FR14k + FR14h cross-epic auto-kickoff amendment (2026-04-21) - closes the Epic 1 iter-95..iter-97 EPIC_DONE re-halt loop where Ralph, after correctly halting at Epic 1 close, re-halted on every subsequent build-mode invocation because the Story-lifecycle decision matrix had only two outcomes for state `done` (next-story or EPIC_DONE halt) and no cross-epic advance path. Amendment adds a third outcome to FR14n's `done` row - when current epic has no more open stories, route via cross-epic transition branch which checks `gh pr view <N> --json state,mergedAt` (gh is source of truth, not the IP text) against the PR recorded in IP § Context and (a) auto-advances by queuing `/bmad-create-story` for Story (N+1).1 if PR is MERGED and sprint-status has a next-epic backlog row, (b) halts `EPIC_DONE` if PR is still open/draft (single-pass - next invocation after merge advances), (c) halts `ALL_EPICS_DONE` if sprint-status has no next epic (terminal, new halt reason), or (d) halts `EPIC_DONE` with a diagnostic `note` if state is inconsistent (autonomy guardrail - no new waiting halt reason, no `AskUserQuestion` in the runtime loop). Extends FR14k halt schema closed reason-set from 6 to 7 by adding `ALL_EPICS_DONE` only (no `AWAITING_USER` - explicitly rejected on autonomy grounds). Adds ninth FR14n anti-constraint: never halt waiting for open-ended human input. Clarifies FR14h `Open | CI green` row: EPIC_DONE is a single-pass halt, re-entry after merge auto-advances. No new FRs; amendments to FR14n + FR14k + FR14h + § Agent Workflow Contracts → Halt schema. New invariant `INV-ralph-halt-reason-enum` registered in INVARIANTS.md + invariants.manifest.ts (shares sourcePath `docs/invariants/ralph-execute.md` with `INV-ralph-halt-path-resolution`; both pin the same contentHash). Runtime enforcement in `.ralph/PROMPT_build.md` § Cross-epic transition + § Halt. `ralph.py _handle_epic_done_transition` gains explicit ALL_EPICS_DONE no-op branch.
---
...
FR14k: System can use the native Claude Code task list as the iteration's crash journal (max 3 active tasks; survives hard kills); agent can hydrate in-flight work via Orient step 7. `.ralph/@plan.md` follows pinned schema (NOW/QUEUE/BLOCKED/DONE/Context; fix tasks enter QUEUE at TOP). `.ralph/halt` sentinel carries pinned JSON schema `{reason, epic, pr}` with closed reason-set at 1.0 (`EPIC_DONE`, `ALL_EPICS_DONE`, `AWAIT_MERGE`, `BUDGET_EXHAUSTED`, `CI_BLOCKED`, `SECURITY_CRITICAL`, `RALPH_STAGE_REGRESSION`). `EPIC_DONE` is a single-pass halt at epic close pending merge; on re-entry, FR14n's cross-epic branch auto-advances. `ALL_EPICS_DONE` is terminal (`epic:null, pr:null`). **Autonomy constraint (non-toggle-able):** every reason is bounded; no reason may block on open-ended human input. Path resolution: when orchestrator runs with `--worktree <name>`, halt/@plan.md/PROMPT/logs resolve against `.claude/worktrees/<name>/.ralph/`, not cwd; orchestrator exports `RALPH_BASE_DIR` (absolute) to subprocess env.
...
NFR33a: The `.ralph/halt` sentinel is a JSON file with pinned schema `{reason: <closed enum>, epic: <N | null>, pr: <url | null>}`; presence halts the loop cleanly. Closed reason enum at 1.0: `EPIC_DONE`, `ALL_EPICS_DONE`, `AWAIT_MERGE`, `BUDGET_EXHAUSTED`, `CI_BLOCKED`, `SECURITY_CRITICAL`, `RALPH_STAGE_REGRESSION`. Autonomy constraint (non-toggle-able): every reason is bounded — self-resolving or triggered by a concrete external condition; no reason may block on open-ended human input. `ralph.py` is the 1.0 reference runtime. Path resolution: sentinel resolves against the worktree when `--worktree <name>` is set; orchestrator exports `RALPH_BASE_DIR` to subprocess env so forks replacing `ralph.py` honour the same path-resolution contract.
...
- **Crash-journal + plan-file + halt schemas (FR14k)**: native Claude Code task list as crash journal (max 3 active; survives hard kills); `.ralph/@plan.md` schema (NOW/QUEUE/BLOCKED/DONE/Context; fix tasks at QUEUE TOP); `.ralph/halt` pinned JSON per R1 (`{reason, epic, pr, iteration_id, timestamp}`; closed enum: `EPIC_DONE | ALL_EPICS_DONE | AWAIT_MERGE | BUDGET_EXHAUSTED | CI_BLOCKED | SECURITY_CRITICAL | RALPH_STAGE_REGRESSION`). `EPIC_DONE` is single-pass (re-entry after merge auto-advances via FR14n cross-epic branch); `ALL_EPICS_DONE` is terminal. Autonomy constraint: no reason may block on open-ended human input. Halt schema at `packages/keel-invariants/src/schemas/halt.schema.json`.
...
As a substrate maintainer,
I want `.ralph/halt` to carry pinned JSON `{reason, epic, pr, iteration_id, timestamp}` with a closed reason enum (`EPIC_DONE | ALL_EPICS_DONE | AWAIT_MERGE | BUDGET_EXHAUSTED | CI_BLOCKED | SECURITY_CRITICAL | RALPH_STAGE_REGRESSION`), schema at `packages/keel-invariants/src/schemas/halt.schema.json`,
...
**And** `reason` is `enum` with values `EPIC_DONE | ALL_EPICS_DONE | AWAIT_MERGE | BUDGET_EXHAUSTED | CI_BLOCKED | SECURITY_CRITICAL | RALPH_STAGE_REGRESSION`.
