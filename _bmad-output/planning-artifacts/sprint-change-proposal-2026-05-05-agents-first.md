...
status: approved 2026-05-06 (Tthew sign-off; Section 6 open items resolved per § Approval). Companion: Proposal A (codex Tier-1 parity, separate proposal).
...
- **`AGENTS.md`'s existing `Promotion rules` section** gains **one inline sentence** reinforcing CLAUDE.md routing (per Section 6 Item 1 decision 2026-05-06): "Claude-Code-specific updates land in CLAUDE.md per the standard promotion rule." **No new "When to update CLAUDE.md" section is added** — the recommendation was theatre layered on the existing rule.
- **`CLAUDE.md`** shrinks to a ≤25-line signpost — carries a **one-sentence INVARIANTS.fork.md precedence pointer** (per Section 6 Item 2 decision 2026-05-06): "Substrate rules take precedence over fork rules. See `docs/invariants/fork.md` § Precedence." Concrete draft in § 4.1.
...
- **Knowledge-file restructure:** AGENTS.md becomes the substrate's primary entry-point. CLAUDE.md shrinks to a ≤25-line signpost with a one-sentence INVARIANTS.fork.md precedence pointer (per Section 6 Item 2 decision).
...
- **R4 rotation flip:** Per-iteration nudge rotates `AGENTS.md / RALPH.md / INVARIANTS.md`. CLAUDE.md drops out of per-iteration rotation. Per Section 6 Item 1 decision, **no new "When to update CLAUDE.md" section is added** to AGENTS.md — Claude-Code-specific updates route via the existing `Promotion rules` table, reinforced by one inline sentence in B-1.
- **Track-of-PRs sequenced delivery (B-internal renumbered B-0..B-1; B-2 deferred to Growth-tier per Section 6 Item 3):**
  ...
  - **B-1** — the actual prose surgery (= original PR #237 PR-3): AGENTS.md absorption + CLAUDE.md shrink (≤25-line signpost with one-sentence fork-precedence pointer) + `PROMPT_build.md` / `PROMPT_plan.md` de-Clauding + R4 rotation flip applied to live files + one-sentence promotion-rule reinforcement in AGENTS.md. Cites already-pinned PRD/arch language from B-0.
  - **B-2** — **deferred to Growth-tier** (Section 6 Item 3 decision 2026-05-06). `docs/ralph.md` + `README.md` codex-aware updates are not pursued at 1.0; revisit when codex Tier-1 (Proposal A) ships or a 3rd CLI lands.
...
- **Scope:** Major. B-0 routes to PM (John, FR14j cited-language signoff) + Architect (Winston, § Knowledge-file contract rewrite signoff). B-1 routes to PM (FR14j cited-language signoff in live files) + Developer (Amelia, execution).
...
3. **B-2 deferred (Section 6 Item 3 decision 2026-05-06).** `docs/ralph.md` + `README.md` codex-aware updates are operator-navigation polish, not load-bearing for substrate correctness. Revisit when codex Tier-1 (Proposal A) ships or a 3rd CLI lands.
...
- **B-1** (M): docs surgery — ~25-line CLAUDE.md (one-sentence fork-precedence pointer per Section 6 Item 2) + AGENTS.md absorption (target ~10-11 KiB content-shape additions) + one-sentence promotion-rule reinforcement (per Section 6 Item 1; no new section, no enumerated triggers) + `.ralph/PROMPT_*.md` line-by-line edits per § 4.2 + R4 rotation flip in any sprint-rotation contract location. **1-2 days.**

**Total elapsed estimate:** ~3-4 days of Ralph build-iterations for B-0 + B-1.
...
**Promotion-rules section — one inline sentence reinforcing CLAUDE.md routing (per Section 6 Item 1 decision 2026-05-06):**

The existing `Promotion rules` table already routes Claude-Code-specific updates to CLAUDE.md. B-1 adds **one inline sentence** in that section: "Claude-Code-specific updates land in CLAUDE.md per the standard promotion rule." No new "When to update CLAUDE.md" section, no enumerated trigger list — the recommendation was theatre layered on the existing rule.

**`CLAUDE.md` target draft (≤25 lines, one-sentence INVARIANTS.fork.md precedence pointer per Section 6 Item 2 decision 2026-05-06):**
...
Substrate rules take precedence over fork rules. See `docs/invariants/fork.md` § Precedence.
...
Per-iteration nudge rotates **`AGENTS.md / RALPH.md / INVARIANTS.md`**. CLAUDE.md drops out of per-iteration rotation — Claude-Code-specific updates land in CLAUDE.md per the standard promotion rule when applicable, not on a rotation cadence. Architecture's R4 contract paragraph (`architecture.md:446`) is updated in B-0 to reflect this rotation set; AGENTS.md's promotion-rules table is updated in B-1 to match (gaining the one-sentence reinforcement per Section 6 Item 1).
...
Although B is docs-prose-only, it touches FR14j (PRD-level) + Architecture § Knowledge-file contract — both PM and Architect must review B-0. B-1's prose surgery routes to PM (cited-language signoff) + Developer (execution).

### Recommended PR sequence (B-internal, B-0..B-1; B-2 deferred per Section 6 Item 3)

| PR | Size | Contents | Depends on | Unblocks |
|----|------|----------|------------|----------|
| **B-0** | M (~1-2 days) | **Foundation docs PR** — PRD FR14j rewrite + FR14k canonical-naming clause + FR40 AGENTS.md cell extension; Architecture § Knowledge-file contract rewrite + R4 rotation contract updated to `AGENTS.md / RALPH.md / INVARIANTS.md`; AGENTS.override.md precedence pinned in FR14j; INVARIANTS.fork.md precedence preservation (one-sentence pointer per Section 6 Item 2) noted as a B-1 acceptance criterion. Pure doc edits to `_bmad-output/planning-artifacts/prd.md` + `_bmad-output/planning-artifacts/architecture.md`; no behavioural change. | None — standalone docs PR. Parallel to A-0 (no shared files). | B-1 cites already-pinned PRD/arch language; A-2 (codex profile) downstream of B-1 reads post-restructure AGENTS.md. |
| **B-1** | M (~1-2 days) | **The actual prose surgery.** AGENTS.md absorbs substrate-truth currently duplicated in CLAUDE.md (target: ~10-11 KiB content-shape additions per § 4.1; doc-budget guardrails govern actual byte size); CLAUDE.md shrinks to ≤25-line signpost with one-sentence INVARIANTS.fork.md precedence pointer (per Section 6 Item 2 and § 4.1 draft); AGENTS.md's existing `Promotion rules` section gains a single inline sentence reinforcing CLAUDE.md routing (per Section 6 Item 1; **no new "When to update CLAUDE.md" section**); `.ralph/PROMPT_build.md` + `.ralph/PROMPT_plan.md` de-Clauded with inline `Claude: X. Codex: Y.` fallback prose per § 4.2; R4 rotation flip applied to AGENTS.md promotion-rules table. | B-0 (cites pinned FR14j, FR14k canonical clause, FR40 AGENTS.md cell, arch § Knowledge-file contract). Independent of A-0/A-1. | A-2 (codex profile correctness) MUST follow B-1 so codex agents read post-restructure AGENTS.md when first running. |
| ~~**B-2**~~ | — | **Deferred to Growth-tier** per Section 6 Item 3 decision 2026-05-06. `docs/ralph.md` + `README.md` codex-aware updates not pursued at 1.0; revisit when codex Tier-1 (Proposal A) ships or a 3rd CLI lands. | — | — |

**Total elapsed estimate (B-only):** ~3-4 days for B-0 + B-1.
...
- **B-1 PR:** AGENTS.md absorption + CLAUDE.md shrink (≤25-line signpost; one-sentence fork-precedence pointer per Section 6 Item 2) + one-sentence promotion-rule reinforcement (per Section 6 Item 1) + `PROMPT_*.md` line-by-line edits per § 4.2.
...
- **B-1:** A non-Claude agent (codex, or any AGENTS.md-respecting CLI) entering the repo for the first time can execute correctly using only AGENTS.md as the entry point — verified by spot-check that AGENTS.md contains the substrate-truth previously sourced from CLAUDE.md (commands table, knowledge-file contract, BMAD invocation, state/crash journal, tool entry points, tool dirs). `wc -l CLAUDE.md` ≤ 25 lines AND CLAUDE.md contains the one-sentence INVARIANTS.fork.md precedence pointer (per Section 6 Item 2). `grep -c "Claude\|TaskList\|TaskCreate\|Sonnet"` in `.ralph/PROMPT_build.md` + `.ralph/PROMPT_plan.md` drops to ≤ 8 hits total (CLI-named adapter callouts only). AGENTS.md's `Promotion rules` section contains the one-sentence reinforcement (per Section 6 Item 1) and **no new "When to update CLAUDE.md" section** has been added.
...
## Section 6 — Decisions (Tthew sign-off 2026-05-06)

| Item | Decision | Rationale |
|------|----------|-----------|
| **R4 upkeep trigger contract content** | **Drop the new "When to update CLAUDE.md" section.** No enumerated trigger list. Add a single inline sentence in the existing promotion-rules section pointing back to the standard rule (Claude-Code-specific updates land in CLAUDE.md per the existing promotion rule — no examples). | The recommended trigger list restated the existing `Promotion rules` table verbatim with examples. Same rule, more prose, no machine-enforced gate. The substantive change is the **rotation flip** to `AGENTS.md / RALPH.md / INVARIANTS.md`; the trigger section was a justification ribbon, not a contract. The existing promotion-rules table already routes Claude-Code-specific discoveries to CLAUDE.md. |
| **`INVARIANTS.fork.md` preservation in the new ≤25-line CLAUDE.md** | **One-sentence pointer.** Literal text: `Substrate rules take precedence over fork rules. See `docs/invariants/fork.md` § Precedence.` | The current verbose form (table row + paragraph at `CLAUDE.md:53` + `:55`) burns ~7 of the 25-line budget on prose that one sentence carries. The detailed precedence rule already lives in `docs/invariants/fork.md` § Precedence — CLAUDE.md just needs the pointer. |
| **`docs/ralph.md` + `README.md` codex-aware updates content (B-2 scope)** | **Defer to Growth-tier.** B-2 is dropped from the 1.0 sequence. Ship B-0 + B-1 only. | B-2 is operator-navigation polish, not load-bearing for substrate correctness. Revisit when codex Tier-1 (Proposal A) ships or a 3rd CLI lands. |

---

## Approval

- [x] **Tthew** — approves Sprint Change Proposal B for implementation (2026-05-06).
- [x] R4 upkeep trigger conditions: **drop the "When to update CLAUDE.md" section; add one inline sentence in the promotion-rules section (no examples).**
- [x] INVARIANTS.fork.md pointer text in new CLAUDE.md: **one-sentence pointer** (`Substrate rules take precedence over fork rules. See `docs/invariants/fork.md` § Precedence.`).
- [x] B-2 **deferred to Growth-tier** — ship B-0 + B-1 only.

Approved. Workflow proceeds to Step 5 (finalise + route) and Step 6 (workflow completion), including the `sprint-status.yaml` update from § 4.4. **§§ 3, 4, 5 of this proposal have been updated to reflect the decisions above; the original recommended forms are superseded.**
