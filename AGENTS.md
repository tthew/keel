...
- **SSH opt-in resolver + compose-override idiom.** The resolver exports `KEEL_DEVBOX_COMPOSE_FILE_SSH` (absolute path to `docker-compose.ssh.yml` when opt-in, empty string when no-SSH). Every compose-invoking shim (build/rebuild/start/stop/status/logs/clean/benchmark — 15 call sites across 8 files) threads `${KEEL_DEVBOX_COMPOSE_FILE_SSH:+-f "${KEEL_DEVBOX_COMPOSE_FILE_SSH}"}` after the base `-f docker-compose.yml`. Agents MUST NOT inline `if [[ $KEEL_DEVBOX_SSH == true ]]` checks outside the resolver and `entrypoint.sh`.
