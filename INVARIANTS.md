...
Git-layer backstop completing the NFR5a/NFR5b three-layer defense: Layer 1 (Story 2.16 in-session hook) catches commit-time attempts; Layer 2 (this H3 — Story 1.9 pre-merge sync-gate over six new contentHash entries, including two seed byte-identity entries landed by Story 2.17 Task 11.1) catches out-of-band attempts that evaded Layer 1; Layer 3 (Story 2.17 Task 5 S4 prompt-injection scan rules — authored but not yet landed) catches attempts that made it into the diff. The sub-tree + names-and-shebangs `hashScope` variants are delivered by Story 2.17 Task 1's manifest schema extension (absent `hashScope` remains back-compatible whole-file sha256 for all pre-existing entries).
