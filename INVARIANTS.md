...
Legacy-devbox branch retains pre-absorption cc-devbox layout as fallback canary during the M0.5 → M4 critical-path window per PRD § Technical Risks bootstrap-handoff mitigation (`prd.md:617`); retired by Story 15b.1's `scripts/major-cut.sh` at the 1.0 cut ritual (`epics.md:6293-6314`). Five workflow contracts pinned: branch creation (upstream fetch + retention banner), cherry-pick (manual minimal-drift; CVE-class / fail-closed-egress / secret-leakage / network-exposure only — no feature-parity), triage (canary-then-bisect — reproduce the regression on the canary; `git bisect start HEAD 5278738 -- packages/devbox/` if absent on canary, escalate upstream if present), sunset criteria (PRD M4 falsification ritual gates retirement), retirement (tag `legacy-devbox-final` + delete active branch + `RALPH.md` decision entry per FR33). Documented-but-not-automated by design; FR44 AMEND required to script cherry-pick. Forks MAY follow the pattern with their own upstream + retention naming OR skip retention if no bootstrap-handoff risk applies; substrate-wins precedence forbids weakening no-feature-parity framing or automating cherry-picks.
...
- **`INV-claude-hook-secret-denylist`** — Claude PreToolUse hook + `.claude/settings.json` `hooks.PreToolUse` block deny secret-access + hook-self-protection patterns; JSONL schema for `blocked-tool-calls.jsonl`; N=3 halt-threshold contract. Story 2.17 Task 4 repoints this entry's `sourcePath` from the invariant doc to the hook script itself (`.claude/hooks/block-secret-access.sh`) — whole-file sha256 of the substrate enforcement script. Invariant-doc drift protection moves to sibling entry `INV-claude-hook-secret-denylist-doc` (Story 2.17 H3). Narrative contract + fork-extension + AMEND path remain at `docs/invariants/claude-hook-denylist.md`.

### Hook + settings bypass-resistance (Story 2.17)

Git-layer backstop completing the NFR5a/NFR5b three-layer defense: Layer 1 (Story 2.16 in-session hook) catches commit-time attempts; Layer 2 (this H3 — Story 1.9 pre-merge sync-gate over six new contentHash entries, including two seed byte-identity entries landed by Story 2.17 Task 11.1) catches out-of-band attempts that evaded Layer 1; Layer 3 (Story 2.17 Task 5 S4 prompt-injection scan rules at `packages/keel-invariants/src/prompt-injection-rules/hook-settings-tamper.ts` — three `high`-severity rules `s4-claude-hooks-tamper` / `s4-git-hooks-tamper` / `s4-skip-permissions-injection` landed iter-329 with 7/7 `node:test` unit tests GREEN; Epic 4 scanner-binary consumer per FR36 wires them to pre-commit) catches attempts that made it into the diff. The sub-tree + names-and-shebangs `hashScope` variants are delivered by Story 2.17 Task 1's manifest schema extension (absent `hashScope` remains back-compatible whole-file sha256 for all pre-existing entries).

- **`INV-claude-hook-secret-denylist-doc`** — whole-file sha256 of `docs/invariants/claude-hook-denylist.md` (the invariant-doc narrative carrying decision-shape + JSONL schema + halt-threshold pin + source-files index + fork-extension + Story 2.17 git-layer backstop table). Sibling to the repointed Story 2.16 `INV-claude-hook-secret-denylist` entry (Story 2.17 Task 4.1 Option B split: ID continuity on the hook-script entry; invariant-doc protection preserved here). Source: `docs/invariants/claude-hook-denylist.md`.
- **`INV-claude-settings-deny-rules`** — `hashScope: jq-subtree` over the two substrate-authoritative sub-trees of `.claude/settings.json` (`.permissions.deny[]` Story 2.15 NFR5a baseline + `.hooks.PreToolUse[]` Story 2.16 hook registration). Filter `{deny: (.permissions.deny // [] | sort), hooks: (.hooks.PreToolUse // [] | sort_by(.matcher))}` canonicalises authoring-order drift; fork-additive edits to `.permissions.allow[]` / `.hooks.PostToolUse[]` / `.hooks.UserPromptSubmit[]` do NOT change the hash. Source: `.claude/settings.json`.
- **`INV-git-hooks-preservation-enumeration`** — whole-file sha256 of `packages/keel-invariants/src/prek-hook-manifest.ts`, the enumerator exporting `EXPECTED_HOOKS: readonly { name: string; shebangPattern: RegExp }[]`. Catches out-of-band edits to the enumeration (e.g. a PR removing `pre-commit` from the set to relax the preservation contract). Source: `packages/keel-invariants/src/prek-hook-manifest.ts`.
- **`INV-git-hooks-preservation`** — `hashScope: names-and-shebangs` with `enumeratorPath = sourcePath` (convention). Walker imports `EXPECTED_HOOKS`, walks `.git/hooks/<name>` for each, and hashes `sort(name\tshebang-line)` joined by newlines. Missing hooks emit `git-hook-missing` drift; shebang-pattern mismatches emit `git-hook-shebang-mismatch`. Pins the contract "these hooks exist AND their shebangs conform to pattern" without binding to byte-bodies that drift across prek upgrades + `.pre-commit-config.yaml` changes. Legitimately shares `sourcePath` with `INV-git-hooks-preservation-enumeration` (distinct `hashScope` canonical form; manifest schema permits). Source: `packages/keel-invariants/src/prek-hook-manifest.ts` (anchor-only — hashed content derived from `.git/hooks/` walk).
- **`INV-claude-settings-seed`** — whole-file sha256 of the seeded copy of `.claude/settings.json` at `packages/keel-templates/src/seeds/.claude/settings.json`. Story 2.17 Task 11.1 substrate-to-seed byte-identity pairing: the seed hash MUST match the substrate whole-file hash of `.claude/settings.json`; any drift pops at Story 1.9 pre-merge sync-gate pointing at whichever file lagged. Whole-file (not sub-tree) because the seed is the full artifact `create-keel-app` (Epic 15a consumer) writes unchanged into a fresh fork — a sub-tree hash would miss fork-extension slot edits that must travel with the seed. Source: `packages/keel-templates/src/seeds/.claude/settings.json`.
- **`INV-claude-hook-secret-denylist-seed`** — whole-file sha256 of the seeded copy of `.claude/hooks/block-secret-access.sh` at `packages/keel-templates/src/seeds/.claude/hooks/block-secret-access.sh`. Story 2.17 Task 11.1 substrate-to-seed byte-identity pairing: the seed hash MUST match `INV-claude-hook-secret-denylist`'s whole-file hash of the substrate; any drift pops at Story 1.9 pre-merge sync-gate pointing at whichever file lagged. `create-keel-app` (Epic 15a consumer) MUST write this seed unchanged (`--preserve-permissions` preserving the exec-bit per D-36 fresh-fork seed contract landed iter-325 Task 13.5). Source: `packages/keel-templates/src/seeds/.claude/hooks/block-secret-access.sh`.
- **`INV-keel-invariants-sync-gate-snapshots`** — `hashScope: anchor-range` bracketing both `EXPECTED_INVARIANT_IDS` (the FIX-3 snapshot) and `BYTE_PARITY_PAIRS` (the FIX-4 snapshot) inside `packages/keel-invariants/src/sync-gate.ts` under a single `:start`/`:end` marker pair (FIX-18, PR #230 Round-4 R4-A6). Closes the bypass class where coordinated 3-leg removal — drop manifest entry, drop INVARIANTS.md anchor, drop `EXPECTED_INVARIANT_IDS` membership — silently slips both pre-existing FIX-3 + FIX-4 backstops if the snapshot bytes themselves are also rewritten. Any content edit inside the bracketed region (member addition/removal/modification, pair addition/removal/modification) fires `content-hash-mismatch` at Story 1.9 pre-merge sync-gate. Recursive-by-design (entry IS in `EXPECTED_INVARIANT_IDS`); contentHash is computed from the post-edit file so the recursion is self-consistent. Pre-existing structural limit at the anchor-range `hashScope` layer (pointer-mutation by retargeting markers to a no-op range with matching contentHash — applies to every anchor-range entry today, not novel here) is mitigated by L1-protection of `invariants.manifest.ts` (`.claude/hooks/block-secret-access.sh` `l1_path_re`) + human PR review; inline WONTFIX comment block above the markers in `sync-gate.ts` records the residual (R4-Inv-I07). Source: `packages/keel-invariants/src/sync-gate.ts`.
