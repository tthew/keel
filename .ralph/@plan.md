...
- [ ] Run `/bmad-testarch-atdd` for Story 1.18 — ATDD scaffolds (FR14n `validated → atdd-scaffolded`); ATDD-skip forecast per § ATDD-skip ground-(a) substrate-verification (the three Python smoke scaffolds ARE the bootstrap red-phase per AC1; FR14n § ground-(b) sunset post-Story-1.17 per issue #233 amendment — cite ground-(a) only OR (a)+(c) variant-(iii) hybrid).
...
- **Story State:** `validated` (SM-validated at iter-364 via `/bmad-create-story (args: "review")`; FR14n state transition `drafted → validated`. 14 PATCHes applied inline — every iter-357 lock-don't-defer rule violation closed; AC5 + Subtask 8.1 byte-pinned AGENTS.md wording; Subtask 4.1 NO `__init__.py` ship; Subtask 5.1 `from tui.theme import theme` direct-import shape; Subtask 6.2 `version: "0.11.7"` exact-pin; Subtask 1.2 `uv pip compile --extra dev` exact command; AC2 parenthetical compressed via SC-9 cross-ref; AC3 `node` job lines 17-29 convention pinned + post-push fallback condition clarified to PR-open via `pull_request:` trigger; AC4 line-23 asymmetry note added; SC-9 closing sentence rewritten to non-Subtask deferral; Tasks-section header parenthetical removed; substrate ledger pruned (pnpm-workspace.yaml + .gitignore rows dropped); architecture.md § M0 line range 198-237 → 198-240 corrected ×6; sprint-status row 142 → 143 + state ready-for-dev. Change Log v1.1. Next NOW = `/bmad-testarch-atdd` for ATDD-skip gate.
