...
- [x] **iter-25 — halt-sentinel re-write #4 (fourth consecutive post-EPIC_DONE re-entry).** Main-repo `/workspace/ralph-bmad/.ralph/halt` present with valid payload `{"reason":"EPIC_DONE","epic":1,"pr":224}` (mtime 11:33 today — carry-over from iter-24 write at `3a3ee21`); main-repo `/workspace/ralph-bmad/.ralph/@plan.md:1` still carries the stale-but-valid-shape `(AWAIT_MERGE — Story 1.6 shipped; PR #222 Open; waiting for user merge)` marker that SHOULD halt ralph.py at `:1758`. PR #224 MERGED at 2026-04-20T11:23:09Z (unchanged from iter-23/24); Story 1.7 `done`; worktree clean; no unpushed commits. Per iter-22/23/24 Gotcha recipe (RALPH.md:100): idempotent re-write of main-repo halt (via absolute path per RALPH.md:88 rule), bookkeep iter-25, commit+push, exit. Fourth data point confirms the recipe is terminal and load-bearing — both halt mechanisms remain armed at main repo between iterations; the clearing-between-sessions race is a ralph.py-side concern outside Story 1.7 scope (open question RALPH.md:120). Future Ralph: if this pattern repeats for a 5th time, escalate to § BLOCKED — the re-entry frequency may indicate an external caller re-invoking ralph.py (cron/loop wrapper) that's outside Ralph's purview.
