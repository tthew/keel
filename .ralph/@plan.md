...
- [ ] Run `/bmad-dev-story (args: "_bmad-output/implementation-artifacts/1-18-bootstrap-python-test-runner-pytest-under-uv.md")` — implement Story 1.18 (FR14n `atdd-scaffolded → in-dev`). Single-iter expected per RALPH.md iter-344 substrate-extension class + iter-359 Story 1.17 second confirmation (10 Tasks / ~22 subtasks; mostly additive edits, no algorithmic rewrites, no multi-site hook-surface lockstep).

## QUEUE (Epic 1 reopen — Stories 1.18–1.21 bootstrap arc per issue #233)
- [ ] Story 1.18 lifecycle continues: atdd-scaffolded → in-dev → traced → sm-verified → done (per FR14n matrix)
...
_(empty — Story 1.18 ATDD-skipped at iter-365 via FR14n § ATDD-skip ground-(a) substrate-verification (bare-(a) sufficiency per Story 1.17 iter-358 precedent; ground-(b) "no test runner" sunset under issue #233 amendment). The three Python smoke scaffolds ARE the bootstrap red-phase per AC1 — they will be authored at `/bmad-dev-story` time and pass on first run by construction (smoke shape per SC-1 + SC-11 pinned-literal assertions). 30th cumulative project ATDD-skip / 2nd post-(b)-sunset / 3rd course-correction-origin.)_
...
- [x] iter-365 (this iter): Story 1.18 ATDD-skipped via FR14n § ATDD-skip clause (no skill invocation per Story 1.17 iter-358 pattern); FR14n state transition `validated → atdd-scaffolded`. Bare ground-(a) substrate-verification sufficiency: every AC ↔ substrate file 1:1 — AC1 literally declares the three smoke scaffolds AS the bootstrap red-phase. Ground (b) "no test runner" sunset under issue #233 amendment. 0 fix-task QUEUE entries. Sprint-status row unchanged. Change Log v1.2. **30th cumulative project ATDD-skip / 2nd post-(b)-sunset / 3rd course-correction-origin.** Cumulative pre-merge PATCH count Story 1.18 lifecycle to date: 14 (unchanged from SM-validate iter-364 — ATDD-skip applies no patches).
- [x] iter-364: `/bmad-create-story (args: "review")` for Story 1.18 — pre-dev SM-validate. FR14n state transition `drafted → validated`. Two-subagent SM review (technical-correctness + prose-density per RALPH.md iter-235 narrow-surface). Subagent A: 2 MUST-FIX + 2 SHOULD-FIX. Subagent B: 5 MUST-FIX + 5 SHOULD-FIX + 3 NIT/PASS — every "decide at dev-story time" deferral locked at SM-validate per RALPH.md iter-357 lock-don't-defer rule (Subtask 1.2 `uv pip compile --extra dev` exact command, Subtask 4.1 NO `__init__.py` ship, Subtask 5.1 `from tui.theme import theme` direct-import shape, Subtask 6.2 `version: "0.11.7"` exact-pin, Subtask 8.1 + AC5 byte-pinned AGENTS.md replacement). **14 PATCHes applied inline at gate** (within iter-363 forecast envelope 8–14; slight overshoot consistent with iter-357's 16-PATCH course-correction-author baseline). 2 deferred (LLM-OPT C.1 Forecast density compression + D.2 Lessons-applied bullet). Sprint-status row unchanged (SM-validate is Ralph-internal). Change Log v1.1. Cumulative pre-merge PATCH count Story 1.18 lifecycle to date: 14.
...
- **Story State:** `atdd-scaffolded` (ATDD-skipped at iter-365 via FR14n § ATDD-skip clause — no skill invocation per Story 1.17 iter-358 pattern; FR14n state transition `validated → atdd-scaffolded`. Bare ground-(a) substrate-verification sufficiency; ground (b) "no test runner" sunset under issue #233 amendment per FR14n. 30th cumulative project ATDD-skip / 2nd post-(b)-sunset / 3rd course-correction-origin. Change Log v1.2. Next NOW = `/bmad-dev-story` — single-iter expected per RALPH.md iter-344 substrate-extension class + iter-359 Story 1.17 second confirmation.)
